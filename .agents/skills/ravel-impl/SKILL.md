...
4. `--merge` のとき、**CI 全緑 + Critical / FAIL 無し**を確認してマージする。
   ...
   **マージ方式はリポジトリの慣習に合わせて squash**（`gh pr merge --squash`）。
   **ただし `--compress` のときは merge コミット**（`gh pr merge --merge`）に
   する。squash すると複数単位が main 上で 1 コミットに畳まれ、
   単位ごとの履歴と revert の単位が消える。まとめた PR で単位ごとに
   コミットを分けさせた意味がそこで失われる
5. マージ後にこの順で片付ける:
   - `main` を更新
...
- **`--compress` で計画書をまたいで束ねること**、しきい値を超えたのに
  確認を取らずに進めること
- **`--compress` の PR を squash でマージすること**（単位の履歴が消える）
