...
- コミット範囲（`A..B`）: そのまま使う。
- ブランチ名 `X`: `$(git merge-base <base> X)..X`。base は PR の base
  ブランチ、なければ `main`。
- PR 番号 `N`: `gh pr view N --json baseRefName,headRefName` で範囲を構成
  （または `gh pr diff N` を直接読む）。
- 引数なし: `$(git merge-base <base> HEAD)..HEAD`。
...
`HERDR_ENV=1` かつ `codex` が利用可能なら独立レビューをさせる:

```bash
herdr pane split --current --direction right --no-focus   # 既存ペインがあれば再利用
herdr pane run <pane-id> "codex"          # 読み取りレビューなので --yolo 不要
herdr wait agent-status <pane-id> --status idle --timeout 60000
herdr pane run <pane-id> "Review task (report findings only, do NOT edit files, do NOT commit): review diff range <range> ... <このチェックリストの要約> ... Reply with numbered findings (file:line, FAIL/WARN, suggested fix) or 'no findings'."
herdr wait agent-status <pane-id> --status idle --timeout 900000
herdr pane read <pane-id> --source recent-unwrapped --lines 60
```

自分の所見と突き合わせ、codex のみが挙げた所見は必ずコードで裏取りして
から採否を決める。利用不可ならスキップし、判定にその旨を記す。
