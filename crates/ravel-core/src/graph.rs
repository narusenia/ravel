...
impl Graph {
    ...
    /// already carries `new_name`, or when a **pairable** port would land on a
    /// parameter key that is already taken — whether or not it brings a
    /// parameter of its own along, because on such a port the name and the key
    /// are the same thing. A port outside the two pairings cannot collide and
    /// is not checked. Renaming a port to its own name is a no-op.
    ///
    ...
    ///
    /// # This moves a graph, not a document
    ///
    /// A parameter key can be named from **outside** the graph: an exposed
    /// parameter declaration binds to a node id plus a parameter key
    /// ([`crate::exposed::ExposedBinding`]), and this method has no way to
    /// reach one. A caller that renames a paired port therefore owes the
    /// document the other half — [`crate::exposed::apply::follow_key_rename`],
    /// in the same Document snapshot — or the project's external contract is
    /// left naming a parameter nothing has.
    ///
    /// That obligation is why this is crate-visible:
    /// [`crate::network::rename_custom_port`] is the only caller, and it
    /// returns the [`KeyRename`](crate::exposed::KeyRename) that discharges it
    /// rather than a bare graph. Widening this back to `pub` would let a
    /// caller outside the crate rename a key with no way of knowing it had to.
    pub(crate) fn rename_port(
        mut self,
        node_id: NodeId,
        side: PortSide,
        old_name: &str,
        ...
        // The collision is checked for every *pairable* port, not only for the
        // ones that carry a parameter today. On a paired port the name IS the
        // parameter key, so landing on an occupied key claims a value the port
        // never declared: a `net.in` GEOMETRY output renamed onto an existing
        // scalar parameter would start reading it and hand a `Scalar` to a
        // GEOMETRY consumer. Refusing the rename keeps the pairing one-to-one
        // whether or not the port had a parameter to bring along.
        if port_is_paired && node.parameters.iter().any(|p| p.key == new_name) {
...
mod tests {
    ...
    fn rename_port_rejects_collisions_and_missing_ports() {
        ...
        // A pairable port with NO parameter of its own is refused just the
        // same: on a `net.in` output the name is the parameter key, so landing
        // on an occupied one would make the port read a value it never
        // declared (here: a GEOMETRY port reading a scalar).
        let unpaired = Node::new(NodeId::new(3), crate::network::NET_IN_TYPE_KEY)
            .with_output("shape", DataTypeId::GEOMETRY)
            .with_param("intensity", ParameterValue::Float(1.0));
        let graph = graph.add_node(unpaired).unwrap();
        assert!(matches!(
            graph
                .clone()
                .rename_port(NodeId::new(3), PortSide::Output, "shape", "intensity"),
            Err(GraphError::DuplicateParamKey { .. })
        ));
        // A free key is still fine, and still moves no parameter.
        let renamed = graph
            .rename_port(NodeId::new(3), PortSide::Output, "shape", "mask")
            .unwrap();
        let node = renamed.node(NodeId::new(3)).unwrap();
        assert_eq!(node.outputs[0].name, "mask");
        assert_eq!(node.parameters[0].key, "intensity");
    }
