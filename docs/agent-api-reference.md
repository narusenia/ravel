...
Geometry        // domains: points / primitives+attrs / instances / detail
    ...
    .sources() -> &[InstanceSource]       // THE source list: images included,
        // in the order `source_index` addresses. Anything that indexes, counts
        // or must not drop a source reads this one
    .set_sources(Vec<InstanceSource>)
    .instance_source() -> Option<&Arc<Geometry>>   // first source when it is a
        // geometry; `None` for an image, rather than renumbering what follows
...
```rust
LayoutParams { size, tracking, leading, align, wrap_width, anchor, writing_mode }
Align::{Left, Center, Right, Justify}    // ::from_name(&str), anything else Left
VerticalAnchor::{Baseline, Top, Center, Bottom}                 // ::from_name
WritingMode::{Horizontal, Vertical}      // ::from_name, ::is_vertical() -> bool
text::TEXT_ALIGNS / TEXT_ANCHORS / TEXT_WRITING_MODES / DEFAULT_SIZE
...
| `vector.length` | CPU | any vector arity → Scalar magnitude. One `type_key` for every arity, since the output type does not follow the input |
| `vector.normalize.vec2` / `.vec3` / `.vec4` | CPU | the input scaled to unit length, same arity out. **The zero vector normalizes to the zero vector** — dividing by a zero length would emit a NaN that spreads silently downstream; a non-finite length takes the same branch |
...
| `field.time` | CPU | emit `FieldValue` answering the evaluation clock: `mode` is `frame` (continuous, `EvalContext::sample_frame`), `seconds` (the default) or `normalized` (frame ÷ `duration`), then `× scale + offset`. `duration` is stated on the node because a field is handed the frame, never the timeline's length; a non-positive one normalizes to `0.0` rather than an infinity. **Always `is_time_dependent()`** — a `FieldValue` is lazy, so this node's re-pull is the only thing that marks `field.apply` downstream as having a fresh input (the trap `field.expression` documents) |
...
- The viewer's playback status line (`INSP-4`) is one label, composed from
  ...
  written only by `PlaybackController::publish_position` (through
  ...
  Which readings are worth showing, and how they read as one line, is
  `ravel_ui::panels::viewer::playback_status_text(PlaybackStatusInputs {
  playing, dropped_frames, selected_factor, effective_factor, cached_frames,
  ...
  The preview element compares the **two factors**, not the two pixel sizes:
  it reports `VRES-4`'s adaptive downgrade (`effective_factor` below
  `selected_factor`) and stays quiet at the factor the user chose, which the
  toolbar already names. The cached share comes from the Timeline's own band
  (`panels::cache_band(cx)`), so the two never disagree; the label sits in
  `LabelPlacement::CanvasTopRight`, the only viewer corner no other HUD holds.
