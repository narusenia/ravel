...
- `export` (export.rs): the export dialog's headless half (`EXPORT-5`).
  ...
  `ravel-cli`'s `--range start:end` is. `ExportError::message_key()` names
  the sentence; nothing here holds prose. The checks are pre-flight only —
  the worker's `check_preconditions` remains authoritative, and
  `available_encoders()` is the one authority on what can be written.
...
- Export (`src/export.rs`, `src/export_dialog.rs`, `panels/render_queue.rs`,
  ...
  (`Completed` / `Failed`) becomes a workspace notification.
