...
| `ui_state.json` | UI 状態（アクティブコンポジション、Timeline の BPM グリッド、コンポジションごとのループ範囲、Properties で畳んだパラメータグループ、ノード本体のパラメータ値を描くか、ビューアのプレビュー解像度係数）。**任意エントリ**で、欠落時はアクティブコンポジションが `root_comp` に、BPM グリッドが既定に、ループ範囲が 0 件に、パラメータグループが全展開に、パラメータ値の表示が**描く**に、プレビュー解像度が **1/2** にフォールバック |
...
`Node` 自体にフィールドを足すのも同じ扱いになる（bincode の位置索引が
`Node` の並びも見るため）。**末尾に足し**、`JOURNAL_FORMAT_VERSION` を上げる。
v11 の `Node.param_groups`（In ノードのパラメータ表示グループ、`PGRP-4`）が
実例で、`.ravprj` も v12 へ上げてある — RON 側は未知フィールドを黙って捨てる
ので、旧ビルドは v12 の文書を読めてしまい、**グループ割り当てを落とした形で
書き戻す**。`Layer.audio` は失えば音が消えて気づけたが、こちらは開いていない
...
旧ビルドは `document/main.ron` の新しい variant をパースできず
...
パラメータ行を描くか、`PGRP-5`）/ `viewer_resolution`（ビューアのプレビュー
解像度係数、`VRES-3`）も版を据え置いている — 既定が
「off / 0 件 / 全展開 / 描く / 1/2」なので、旧ビルドが捨てても
**画面を見れば分かる**。

`viewer_resolution` だけは読み手が**さらに寛容**で、読めない値
（手編集の綴り間違い）はそのフィールドだけ既定に落ちる。他のフィールドは
`serde` の失敗がそのまま `ui_state.json` 全体の破棄になる
（`ProjectFile::from_archive` が警告して既定で開く）ので、
**列挙型を足すときはこの形に倣う** — 係数 1 つの綴り間違いで
アクティブコンプもループ範囲も失うのは釣り合わない。
