...
**警告は失敗ではない。** どれが出ても終了コードは変わらないので、
バッチから「黙って間違った出力」を弾くにはこの `id` を読む。

| `id` | 意味 | 粒度 |
|---|---|---|
| `audio-not-rendered` | 音声を持つ合成を映像のみで書き出す（`--no-audio`、またはこのビルドに FFmpeg が無い） | レンダーごとに 1 行 |
...
持てないので（`docs/implementation/render-warning-channel-plan.md`）、
