...
| `image-instancing-plan.md` | `InstanceSource`, the `geometry.from_image` node, and the rasterize texture path — frame buffers copied through the existing instance mechanism; retires `SceneContent::Image` | — (IMG-1 merged; roadmap phase C4 is complete, so IMG-2 is unblocked and IMG-3 onward follows IMG-2) | REQ-3D-001, REQ-MOGRAPH-001, REQ-CORE-010 |
