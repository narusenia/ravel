...
| `layer-content-size-plan.md` | A size on the layer's content — `shape.rect` gains an `auto` / `fixed` resolution mode so a Solid can be 200x200 instead of always covering the composition, the rows `auto` drives go read-only, and the three bbox paths that miss image-instance rects, instance positions and stroke width are closed. **Touches no persistence format; the raster extent (RoD) is deliberately a later plan** | — | REQ-UI-002, REQ-UI-013, REQ-LAYER-008 |
