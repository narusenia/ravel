...
| `instance-pieces-plan.md` | Distributing the **instances** of one source across `scatter.*`'s points instead of cloning it whole — a text node scattered per character, with the per-character attributes carried onto the output so a stagger still works. The fan-out machinery already exists and only triggers on wire count; this adds the explicit mode that feeds it | — | REQ-CORE-010, REQ-MOGRAPH-001, REQ-MOGRAPH-004 |
| `layer-content-size-plan.md` | A size on the layer's content — `shape.rect` gains an `auto` / `fixed` resolution mode so a Solid can be 200x200 instead of always covering the composition, and the rows `auto` drives go read-only. **Touches no persistence format** — the default stays `fixed` so no stored shape moves, and both the raster extent (RoD) and the three bbox paths that mis-measure content (`MED-APP-45`, `MED-CORE-11`, `LOW-APP-33`) are deliberately out of scope | — | REQ-UI-002, REQ-UI-013, REQ-LAYER-008 |
| `node-transform-section-plan.md` | A transform declared on the node instead of wired after it — one `with_transform_section()` on a template grows `geometry.transform`'s own `translate` / `rotation` / `scale` / `pivot` parameters, and one wrap in `processor_for_node` applies them, so placing a `text.layout` or a `geometry.from_image` stops needing an extra node. **No synthetic node**: the parameters sit on the node the user already sees, which keeps cache identity and the node editor untouched | `text.layout` の `position`（`TFORM-2` 以降） | REQ-UI-002, REQ-UI-011, REQ-MOGRAPH-001, REQ-MOGRAPH-004 |
...
| `gpui-kit-migration-plan.md` | Adopt `gpui-kit` 0.6 so `gpui-base` (unstyled behaviour) can carry Ravel's own components, keeping gpui-ce as the `gpui` a forked kit points at — **the gate (`KIT-0` / `KIT-0b`) has passed: `gpui-component`'s residue is 38 with zero structural divergence, and `gpui-base`'s own tests are 761/764 against the fork** | `KIT-1` next (swap Ravel's base) | REQ-UI-002/003/013, REQ-GPU-001 |
...
|---|---|---|---|
| `done/geometry-drawn-bounds-plan.md` | One function that measures what a geometry **draws** — point positions, every instance's source placed through its own `InstanceTransform` (an image's rect, a glyph's outline), and the stroke's reach — and the control hull of its curves — so the core and the Viewer stop disagreeing about what a bbox is. Closes `MED-APP-45` and `MED-CORE-11`, and with them the `text.layout` bbox that is a zero-height line until a `text.to_path` is inserted. **`LOW-APP-33` stays open for its other half**: the reach of a `stroke_width` *attribute* is measured, but the `rasterize` node's base parameter is not something a geometry can see | #547 — 2026-09-18 | REQ-UI-013, REQ-MOGRAPH-001, REQ-MOGRAPH-004 |
| `done/contextual-parameter-options-plan.md` | Contextual option lists and a parameter-driven output type: `layer.ref` picks a sibling layer by name instead of scrubbing an id (`.ravprj` v13), its `port` is a dropdown of the referenced layer's output ports, and the node's **output type follows that port** — so a reference to a geometry port stops declaring a frame. The mechanism (`ParamOption`, `ParamOptions`, `contextual_options`) replaced the packed `"3: Background"` option format everywhere, the Parent picker included. Closed `MED-APP-29` | #542, #544, #546 — 2026-09-17 | REQ-LAYER-005, REQ-UI-002 |
| `done/ui-component-layer-plan.md` | The `ravel-widgets` crate holding Ravel's own `Icon` / `Button` / `Tooltip` / `Input` / `NumberInput` / `Checkbox` / `ColorPicker` on `gpui-base`'s unstyled primitives, one source for the colour / spacing / row / type / motion tokens with the literals banned by the pattern lint, a user theme directory, and the twelve UX invariants — every issue violating invariants 1–9 is closed except `MED-APP-29`, which moved to `contextual-parameter-options-plan.md` because it needs contextual parameter options rather than a panel fix | #517, #519, #521, #522, #526, #528–#534, #536, #537, #540, #541 — 2026-09-15 | REQ-UI-002, REQ-UI-013 |
