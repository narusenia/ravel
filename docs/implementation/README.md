...
| `zero-copy-viewer-plan.md` | Draw the viewer straight from the GPU texture — the last of `HIGH-09` (the GPU→CPU→GPU round trip). **`ZC-1` is a measurement gate**: the round trip is what is left after `CM-7` moved the display transform to the GPU, and the plan is frozen if removing it buys less than 5% of the 60 fps budget | `color-management-plan.md` (CM-7, merged), `gpu-backend-plan.md` (GPUBK-9) | REQ-GPU-001, REQ-INFRA-009, REQ-UI-004 |
