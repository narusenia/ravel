...
| `ui-component-layer-plan.md` | A `ravel-widgets` crate for Ravel's own parts, one place for colour / spacing / type / motion tokens with the literals banned by the pattern lint, and the twelve UX invariants the open panel bugs each violate — **`UIX-0` / `UIX-1` (invariants and token values) run alongside `KIT-1`; the parts themselves wait for `gpui-base`** | `gpui-kit-migration-plan.md` (`KIT-1` for `gpui-base`) | REQ-UI-002/013 |
