...
| `gpui-kit-migration-plan.md` | Move Ravel's GPUI base from gpui-ce to the published `gpui-pre` so `gpui-base` (unstyled behaviour) can carry Ravel's own components — **`KIT-0` is the gate: port the fork's nine surface/device patches and prove zero-copy still draws** | — (the fork's patches must port first) | REQ-UI-002/003/013, REQ-GPU-001 |
