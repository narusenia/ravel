...
| `geometry-drawn-bounds-plan.md` | One function that measures what a geometry **draws** — point positions, every instance's source placed through its own `InstanceTransform` (an image's rect, a glyph's outline), and the stroke's reach — so the core and the Viewer stop disagreeing about what a bbox is. Closes `MED-APP-45`, `MED-CORE-11` and `LOW-APP-33`, and with them the `text.layout` bbox that is a zero-height line until a `text.to_path` is inserted | — | REQ-UI-013, REQ-MOGRAPH-001, REQ-MOGRAPH-004 |
