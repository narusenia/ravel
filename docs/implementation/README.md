...
| `color-management-plan.md` | Linear working space, per-asset input colour space, viewer/export transforms, then the OCIO backend — **`CM-1`–`CM-5` are implemented (the pipeline is linear); `CM-6`–`CM-8` (the `ocio-rs` backend) are not started** | `done/render-export-plan.md` (EXPORT-1 for CM-4) | REQ-RENDER-003, REQ-CORE-009 |
