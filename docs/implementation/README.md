...
| `wrangle-plan.md` | A multi-statement, multi-attribute CPU wrangle node and spare parameters on any node — **post-release. `WRG-1`–`WRG-3` (language, node) have no gate; `WRG-4`–`WRG-6` (spare parameters) wait on `HIGH-30` and on the undecided exposure-model branch** | `issues/high/HIGH-30` | REQ-CORE-010, REQ-CORE-015, REQ-PLUGIN-005 |
