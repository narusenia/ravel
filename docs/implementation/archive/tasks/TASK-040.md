...
## 完了条件
- [x] フィールドがノード間を流れ、合成できる（PR #58）
- [ ] ビルトイン5種 + 合成4種が動作する（3/5 実装 — ノイズ/フォールオフ/カーブリマップ。
      Lua 式・画像サンプラは placeholder、合成4種は実装済み）
- [x] 任意のジオメトリ属性をフィールドで変調できる（`field.apply` で
      Geometry + Field → Geometry を配線済み）
- [ ] 統一チャネルの値ソースとして接続できる（未着手）
- [x] 同一 seed で決定的（テストで担保）（PR #58）
