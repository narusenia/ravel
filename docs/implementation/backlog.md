...
`SET-1` / `SET-2` は済み（#276 / #275）。**`settings.toml` に `locale = "ja"` を
書けば UI が日本語になる** — `ja.toml`（235 キー）への到達経路が開通し、
`MED-APP-10` の中核が消えた。起動時に `default → global → project` が解決されて
`AppSettings` Global に載り、層ごとに独立した書き込み API（失敗は通知）がある。
設定ダイアログは 2 画面（環境設定 = `global` 層 / プロジェクト設定 = `project` 層）の
骨組みまで入っており、`groups_for` が `SET-3`〜`SET-6` の拡張点。
**項目はまだ 1 つも無い**ので、サイドバーは `SET-3` が最初のフィールドを置くまで空。
`user` 層は未実装のまま（マシン固有 / CLI 上書き用の枠）。
