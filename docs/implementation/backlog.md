...
| UIX-5 | ⬜ | 行高を 2 段に統一（`row.compact` 20 / `row.default` 24）。**compact の字送りもここで決める** — `UIX-4b` で `.xsmall()` の `text_xs`(12px) が失われ、両段が `text.font_size`(14px) になった。20px の行に 14px は DCC の慣習より大きい。トークンが 1 つ増えるので `UIX-4b` では発明しなかった | UIX-3 |
