...
| UIX-6 | ✅ | 不変条件 1〜4 の違反を潰す（`MED-APP-03` / `04` / `05` / `06` / `07` / `LOW-APP-02`）。**同じ規則の兄弟の腕も塞いだ** — `MoveKeyframe` / `GraphKeyframes` の「動いて戻った」ジェスチャ（`ux.md` が「名指しの 1 経路だけ直すと兄弟は壊れたまま」と言う形）。**残った隙間は `LOW-APP-29` / `LOW-APP-30` に切り出した**（デルタでは検出できない no-op。修正方針は押下時スナップショットと `drag_ended` 時点の文書の比較） | UIX-0 ✅ |
| UIX-7 | ✅ | 不変条件 5〜9 の違反を潰す。**2 PR に割った** — 局所の 3 件（`MED-UI-07` 狭い幅で Vector 行が到達不能 / `MED-APP-17` 効かない Fit ボタン = 縦ズーム未実装 / `MED-APP-08` identity の合わないサムネイルを捨てていない）と、不変条件 7 の 3 件（`MED-APP-19` / `MED-APP-20` / `MED-APP-44`。根は 1 つで、レジストリに「色かベクタか」の宣言が無いこと）。**`MED-APP-29` はこの単位では閉じない** — 2026-08-09 に `contextual-parameter-options-plan.md`（`CPO-1`〜`CPO-7`）へ移した票で、個票が「その単位が入った時点で閉じる」と言っている | UIX-0 ✅ |
...
読んでいた。前送り後の実際の残差は `gpui-base` が 13、`gpui-component` が
38（しかも 38 は下限）。詳細は計画書の「`KIT-0` の実測」節。
...
載せる）。`edited_string_param` が既存の型を見ずに `ParameterValue::String` を
返すので、`layer` が Int のまま Select になるとフォーマット版を上げないまま
`.ravprj` の中身が変わってしまう。

**移行は id 予約より前に走らせること。** `layer_ref_targets` はテキスト綴りを
読むので、`Int` のままの文書では参照が 1 つも見えない。`advance_id_counters()`
より後に移行すると、予約されていない生きた参照が残り、dangling な参照が
...
`hash(seed, index)` で振る）は**既にあり**、2 本以上のワイヤでしか起動しない
のが原因。1 本のソースが持つインスタンスをピースとして扱うモードを足す。
パラメータは **`piece_mode`**（`whole` 既定 / `instances`）で、`source_mode`
（配る順）とは別の軸。ピースは**インスタンスごと**（`sources()` は重複排除
されているので "aa" が潰れる）、ソースの `P` は捨てて `rot` / `scale` は
焼き込む、`instances` は全ワイヤを 1 本のピース列に平らにする。
**per-character 属性を出力へ写すのが眼目** — 写さないと配れてもスタガーが
