...
> **Status**: Done (#65, #66, #67, #68, #69) — 2026-07-17
...
**設計決定（Phase 2 冒頭 gate、実装前に確定）**:

- (a) と (b) の折衷を採用する。ravel-core への依存注入は trait
  オブジェクトではなく **`NodeData::is_gpu_resident()`（default =
  false）** の 1 メソッドに縮退させる。dyn NodeData から第二 trait への
  アップキャストは `as_any` 経由の具象型知識を要求し、コアの型非依存を
  破るため。コア（永続化・将来の三層フレームキャッシュ）はこの述語だけで
  「GPU 常駐値は直接永続化できない」判定ができる。
- 具象型 `GpuFrameBuffer` は **ravel-gpu**（`frame.rs`）に定義。
  ravel-gpu → ravel-core の既存依存方向のみで完結し、逆依存は生じない。
  `DataTypeId` は `FRAME_BUFFER` を共有（ポート型・エッジ検証は不変）し、
  CPU/GPU の表現差は processor 境界のヘルパで吸収する（(b) の要素）。
- `GpuFrameBuffer` は `GpuContext` クローンと共有プールへの `Weak`
  参照を内包する。これによりどの境界からでも追加のコンテキスト
  プラミングなしに読み戻しでき、最後のハンドル drop で texture が
  プールへ自動返却される（`Arc<PooledHandle>` 内包で Clone 安全）。
- テクスチャプールは processor 毎の `Mutex<TexturePool>`（各 256 MB）を
  廃止し、**eval ワーカー単位の共有 `Arc<Mutex<TexturePool>>`** に統合
  （`register_all_processors` / `processor_for_node` の引数に追加）。
  常駐ハンドルがプール寿命を越えないよう Weak で参照する。
- GPU ノード内の dispatch 毎 `ctx.wait()` は除去し、同一 queue の投入
  順序保証に委ねる。ブロッキング待ちは `read_texture`（境界）のみ。
- 転送カウンタはプロセス global から **`GpuContext` インスタンス毎**に
  移す（テストが自分のコンテキストの転送数を並行テストと干渉せず検証
  できるようにする）。
...
実装ノート（Phase 4 最低ライン完了時追記）:

- 読み戻し 1 回は Phase 1/2 の時点で **評価ワーカーの
  `GpuEvalHooks::finalize`** に置いた（UI スレッドでの読み戻しは
  ブロッキングに逆戻りするため、計画の字面より安全側）。よって
  `ViewerFrame` は CPU `FrameBuffer` のまま。
- ViewerPanel は `ViewerFrame` 更新時に 1 回だけ straight-alpha BGRA u8
  の `RenderImage` へ変換（`frame_to_render_image` span 計装済み）し、
  `img` + `ObjectFit::ScaleDown` で表示。render() 毎の run-merge 走査と
  quad 提出（グラデーションで 262,144 quads = ピクセル毎）が
  **1 テクスチャ quad** になる。
- `ImageSource::Render` は gpui の image cache を経由しないため、
  atlas エントリは明示的な `drop_image` でしか解放されない。ViewerPanel
  はフレーム差し替え時に旧 `RenderImage` を `cx.defer` + `drop_image`
  で全ウィンドウから退去させ、パネル解放時（`on_release`）に最終
  フレームも解放する。この掃除が連続更新（再生）での VRAM 安定の
  前提条件。
- 再生（TASK-013 step 5）への接続点: `ViewerFrame` の発行元
  （`NodeEditorPanel::on_eval_update`）を再生クロック駆動の評価要求に
  差し替えるだけで、表示経路は上記の退去処理込みで連続更新に耐える。
  変換コストが問題になれば worker 側（finalize）へ移動可能。
  ゼロコピー（ストレッチ）は未着手のまま。
