...
> **Status**: 完了 — 2026-09-18（`BBOX-1` / `BBOX-2` / `BBOX-3`、#547）
>
...
- **曲線はアンカーだけでは測れない。** `in_tan` / `out_tan` を持つパスは
  cubic がアンカーの外へ膨らむ（`rasterize::path_polyline` が
  `flatten::flatten_path` へタンジェントを渡して描く）。**制御点の凸包**で測る
  — Bézier は制御多角形を出ないので小さすぎることがなく、列の走査 1 回で済む
  （セグメントごとに導関数の根を解かない）
...
- タンジェントがアンカーの外へ出るパスの bbox が、**描かれる曲線の頂点を含む**
  （水平に並んだ 2 アンカー + 上向き 60 のハンドルは `y(t) = 180t(1-t)` で
  45 まで届く。アンカーだけなら高さ 0）。凸包なので答えは 60 でよく、
  テストが固定するのは「45 を下回らない」こと
...
5. **曲線は制御点の凸包で測る**（独立レビューの指摘、2026-09-18）。
   セグメントごとに cubic の極値を解く案は却下した — 列の走査が
   セグメントごとの根解きになり、ポインタ移動ごとに払うコストの約束
   （`drawn_bounds` の doc comment の表）を崩す。凸包は**小さすぎることがなく**、
   **text にはコストゼロ**（フォントは極値にアンカーを置くので、グリフの凸包と
   インクは同じ矩形。Geist Regular の `"Ravel"` 72px で小数まで一致を実測）。
   曲線が自分のハンドルまで届かない場合だけ広く出る
6. **`LOW-APP-33` は半分だけ閉じる**（同日）。`rasterize` の基底
   `stroke_width` は**ノードパラメータ**（`rasterize/mod.rs:329`）で、
   ジオメトリ 1 つを引数に取る関数からは原理的に見えない。属性由来は入るので
   issue を「半分解決 + 残りを名指し」に書き換えて open のままにした
