...
`"rgb"` / `"a"` のような成分名リストとし、既定は全成分。成分は位置で
解決するので `x`/`r`、`y`/`g`、`z`/`b`、`w`/`a` は同じ枠を指す。
**対象型に存在しない成分だけを指定した場合**（`Vec2` に `"z"` 等）は
黙って no-op にせず、group 名と同じく警告して全成分へフォールバックする。
...
`apply_field` にモード引数を足す。`Set` / `Add` / `Multiply` / `Min` / `Max`
の **5 種**。`amount` は合成結果への補間率として全モード共通に作用する。

```text
result = existing + (combine(existing, sampled) - existing) * amount
```

既定モードは `Set`。`Set` はこの式に入れると
`existing + (sampled - existing) * amount` となり、置き換え前の
`blend_arrays` と**項まで一致**するので既存の呼び出しはそのまま通る。

> **当初案からの変更（実装時 2026-07-27）**。当初は `Blend` を含む 6 種とし、
> `Blend` のみ補間率、他は `existing op (sampled * amount)` と規定していた。
> しかしその式では `Multiply` の `amount = 0` が `existing * 0 = 0` になり、
> 本節が掲げた「`amount = 0` はどのモードでも変調なし」を `Min` / `Max`
> ともども満たさない。合成結果へ補間する形に改めると不変条件が全モードで
> 成立し、同時に `Blend` は `Set` + `amount` と完全に一致して不要になる。
> 表現力は落ちない（旧 `Blend` = 新 `Set`）。

`amount = 0` は**ビット等価**で「変調なし」にする。ゼロ補間でも合成演算を
先に評価すると `-0.0` が `+0.0` に化け、中間値が溢れた場合は
`inf * 0 = NaN` になるため、`amount = 0` は入力列をそのまま返す。
...
- 全 `CombineMode` × `amount = 0.0` で入力ジオメトリと一致するテスト。
  一致は**ビット比較**で見る（`-0.0` と NaN payload を含める。数値比較だと
  `-0.0 == 0.0` と `NaN != NaN` が退行を隠す）。
- `group` 未指定で既存の `field.apply` テストが無改変で通る
  （既定 `Set` + 全要素の後方互換）。
- group 内のみ変化し、group 外が**バイト等価で不変**であるテスト。
  非選択成分（`rgb` マスク時の `a`）についても同じくビット比較。
- 存在しない group 名 / Bool でない属性が全要素フォールバックになる
  （`Err` ではない）テスト。
- 対象型に無い成分だけを指定したマスク（`Vec2` に `"z"`）が全成分
...
> **`P` 拒否はドメインで閉じる（実装時 2026-09-03）**。`P` の削除を拒む
> のは `Point` / `Instance` の 2 ドメインだけで、名前だけを見て全ドメインで
> 拒むのではない。`Geometry::validate` が `P` を要求するのはこの 2 つで、
> `Primitive` / `Detail` の `P` は誰も読まない普通の列だから。**位置列が
> そのドメイン最後の列だった場合**、削除は validate を壊すのではなく
> `element_count()` を 0 に落として黙ってドメインを空にするので、
> ドメイン単位の無条件拒否にしてある（要素数 0 のときだけ許す、にはしない）。
>
> 削除後の `validate()` は現状どの経路でも `Ok` を返す（列を落とすことで
> 壊れる不変条件は `P` の 2 件だけで、それは事前に弾いている）。
> それでも呼んでいるのは他の `ops` と形を揃えるためで、削除で壊れる
> 不変条件が将来増えたときに 1 箇所で拾えるようにするため。
...
> **この一文は 2026-09-03 に事実と違うと判明した。** REQ-CORE-010 の受入条件に
> 「削除」の項目は無い（削除は説明文にしか出てこない）。「式から属性値を
> 参照できる」も REQ-CORE-015 の式で既に達成済みで、単位 4 の前から
> チェックが入っていた。REQ-CORE-010 の残り 6 条件は既存ノードで満たされて
> いるように見えるが、要件 1 本の充足確認は本計画の範囲外である。
...
| パラメータに式を設定できる | 達成済み（本計画の外） | — |

**結果は 7 条件すべて達成**（2026-09-03、単位 5）。「式」は REQ-CORE-014 の
専用スカラー式言語で、**Lua ではない**（REQ-CORE-007 の 2026-08-03 改訂で
置き換わった）。計画時点の「Lua 式は未達のまま」は、この改訂前の記述である。
Lua は REQ-CODE-001 のコード Layer 専用で未達だが、per-instance 変調は
それに依存しない。
