...
> **Status**: 14 単位すべてマージ済み（`RESP3-1`〜`4` は #395、`RESP3-5`〜`11`
> は #397、`RESP3-12`〜`14` は #396）。**`RESP3-7` だけ完了条件が未達**なので
> この計画は `done/` へ移していない。
>
> closed になったのは `HIGH-01` / `HIGH-02` / `MED-CORE-01` / `MED-CORE-05` /
> `MED-UI-01` / `MED-UI-03` / `MED-UI-04` / `MED-UI-06` / `MED-GPU-04` /
> `MED-GPU-05` の 10 件。`MED-UI-05` も **#400 で closed**（この計画の外で
> MediaBin 側を片付けた）。`MED-UI-02` も **`VIS-2`〜`VIS-4` で closed** —
> 残っていた「パネル非表示のときスキップ」を
> [`panel-visibility-plan.md`](panel-visibility-plan.md) が入れた。
> **未解決の項目は無く、フェーズ C3 は `完了`。**
> 実施結果の要約は `roadmap.md` フェーズ C3 の `実施結果`。
>
...
> **完了条件が未達（#397 マージ後）。** 入ったのは「プレイヘッドで再サンプル
> されるものが何も無いときスキップ」で、静的ターゲットの再生は 30 → 1 回に
> なった。アニメーション有りのターゲットは 30 回のまま据え置きで、これは
> 正しい（削ると値が止まる）。
>
> **残っていた「パネルが非表示のとき `refresh_values` を走らせない」は
> `VIS-2` で入った。** タブの可視性がパネルへ届く経路が無かったのがこの単位で
> やらなかった理由で（`ravel-dock` はタブの `active: usize` を持つがそれを
> `PaneContent` 側へ伝えない）、配線はドッキング層とパネル層をまたぐ設計ゲート
> 規模の作業として [`panel-visibility-plan.md`](panel-visibility-plan.md) に
> 分けた。`VIS-4` の完了でフェーズ C3 も `完了` になった。
...
> **実測メモ（#397）**: 前提の一部が違っていた。**折り畳まれたレイヤーでは
> 行モデルの構築は元から走っていない**（100 枚折り畳みで 0 回）。したがって
> カリングで消えたのはヘッダのウィジェット構築とキャンバス描画で、
> `keyframes::property_rows` の計算ではない。可視 y 範囲はヘッダ側が
> `ScrollHandle`、painter 側が `window.content_mask()` から取る。
...
- **Timeline の行レイアウトをキャッシュしない**（`RESP3-8`）。無効化の条件を
  1 つ落とすだけで行がずれた位置に描かれる。可視 y 範囲を毎回求め直す方が
  安全で、実測でも足りている
- **`DISTANCE_WEIGHTED_NEIGHBOURS` をパラメータにしない**（`RESP3-4`）。
  ノードのシグネチャ変更になる。8 近傍が全域 IDW より高精度である以上、
  ユーザーが触る理由も薄い
- **`PathInterner` に eviction を入れない**（`RESP3-3`）。現状パスの集合は
  ドキュメントの構造に対して有界で、無限に増える経路が無い。増やすのは
  `PathSegment::TimeShift` を配線する単位（`SCOPE-2`）なので、
  **eviction はその単位が引き受ける**
