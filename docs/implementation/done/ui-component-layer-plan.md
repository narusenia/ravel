...
`yield_to_open_menus` の除外リストに入れる。**

**当初これを `UIX-10` として単独の単位にしたが、2026-09-09 に `UIX-11` へ
畳んだ。** 理由: 実測すると **`←` / `→` を取る要素が 1 つも無い**
（`on_key_down` は全部 enter / escape のインライン改名、`curve_editor` に
矢印の微調整は無い、`↑` / `↓` を使う `node_editor/palette` と
`NumberInput` は未束縛なので衝突しない）。`Input` のキャレット移動は
**既に `!Input` の除外が守っている**。

だから文脈を先に作ると**それを載せる要素が 1 つも無い**状態になり、
テストできるのは文脈の文字列だけになる —— 不変条件 8 が warn している
「消費者の無い場」そのものである。**最初の消費者（2D 彩度面）と同時に
入れれば、「矢印で彩度が動き、再生ヘッドは動かない」を挙動で証明できる。**
...
| 7 | **値の意味が編集器に出る** — 成分名・単位・型 | `MED-APP-19` / `MED-APP-20` / `MED-APP-29` / `MED-APP-44` |
...
| `UIX-11` | **`color_picker` を自前で作る**（2D 彩度面 + 色相帯 + A + hex、`pattern_slash` のスウォッチ）。**既存の undo の契約を壊さない**（即時適用 + 静穏期間後に 1 段）。**矢印キーを所有する要素のキー文脈もここ**（下記） | `UIX-4b` |
...
| 5 どの幅でも | `MED-UI-07` |
| 6 死んだ操作 | `MED-APP-17` |
| 7 値の意味 | `MED-APP-19` / `MED-APP-20` / `MED-APP-29` / `MED-APP-44` |
...
- ~~**gallery の目視を誰がやるか。**~~ **解決**（2026-09-10）。
  `screencapture` は通る（TCC の拒否は当時の環境の話で、今は
  エージェントも絵を確認できる）。`UIX-9` の `Checkbox` は人間が実機で
  目視して良好と確認した。**実機の目視は人間の作業として残す**という
  結論自体は変えない — 絵が「読めるか」の判断は自動テストの外
- ~~**`Input` を借り続けることの代価。**~~ **解決**（`UIX-4b`）。
  `Input` と `NumberInput` は `gpui-base` の無スタイル primitive に
  載せ替えて Ravel 自前になったので、質感の不揃いは消えた
