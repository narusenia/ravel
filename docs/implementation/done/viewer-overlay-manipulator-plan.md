...
> **Status**: Complete — 2026-08-15（単位 1〜9 と単位 6 のロケール / 文書更新まで完了）
...
- **サンプル上限 `MAX_MOTION_SAMPLES`（256）を超えない。** 区間に**両端込みで
  等間隔に配る**（`in + i·(last − in) / (N − 1)`）ので、ちょうど 256 点で
  末尾フレームも含む。**stride で刻んで末尾を後付けする形は上限を 1 超える**
  （span 512 で 257 点になった）。境界（上限の倍数 ±1）のテストで固定する。
  キー点は `stride_for` で間引き、こちらは `ceil` 刻みなので超えない
...
- **group は Bool 属性**（`procedural-geometry.md` 要素スコープ）。色は
  **group 名のハッシュ**から取るので、並ぶ group が増えても domain が違っても
  同じ group は同じ色。**色相だけでなく彩度・明度にもハッシュを配る** —
  色相だけでは 360 バケットしかなく、500 名で 258 色に潰れた（実測）
- **矢印の対象属性はピッカーで選ぶ**。候補は評価済みジオメトリが実際に持つ
  Vec2 / Vec3 / Vec4 列（`P` は除く）で、予約名の固定リストではない。
  `particle-plan.md` の velocity 表示はこれに乗るだけで専用経路を持たない
- **属性列はコピーせず借用して index で読む。** ピッカーは型だけを見る。
  100 万点で 1 render あたり picker 758 µs → 1 µs、矢印 296 µs → 17〜22 µs
  （実測、release）。列を丸ごとコピーすると上限 2000 マークとは無関係に
  ジオメトリ全長の割り当てが UI スレッドに乗る
...
| ボックス選択（ラバーバンド）の枠 | `viewer-tool-extensions-plan.md` の `TOOLX-2`（枠の描画は本機構に乗る） |
