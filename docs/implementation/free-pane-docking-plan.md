...
> **Status**: In progress — 2026-08-01 `DOCK-1`〜`DOCK-8` 実装済み（#229, #234,
> #231, #235, #228, #236, #238, #240）。カットオーバー済みで、残りは `DOCK-9`
> （永続化）と `DOCK-10`（実機確認と文書更新）
...
- `title_bar.rs` を全窓共通コンポーネント化: 共通部（ドラッグ領域 +
  ウィンドウコントロール + 中央寄せ補正）+ 窓種別スロット。メイン =
  プロジェクト名、分離 = 窓タイトル（単一パネルならパネル名、複数タブ時は
  「N panels」形式のロケールキー）+ AlwaysOnTop ピン。
  **プリセット切替ボタンは TitleBar から外す**（2026-08-01 決定。切替経路は
  `Cmd+F1`〜`F4` と Workspace メニューのまま。バーの役割を窓の識別に絞る）。
