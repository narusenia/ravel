...
| ディスパッチ 1 件の encode | 158 µs / 評価（30.26 パス） | **ほぼ取れる。** 5.06 µs/パスの傾きは「ディスパッチごとに `begin_compute_pass`」という**我々のコードの構造**由来で、同じ wgpu で 1 パスに畳むと 0.09 µs/パス。残り約 **20 µs / 評価**だけがバックエンド固有 |
| 自動バリア挿入 | **測定誤差内（0）** | 空振り。Metal 側でエンコーダ境界を刻んだ場合と 1 エンコーダに畳んだ場合が全チェーン長で誤差内 |

**バックエンドを書いて初めて取れるのは約 20 µs / 評価**、60 fps 予算 16.7 ms の
**0.1%**。Metal / D3D12 / Vulkan の 3 本を**永続的に保守する**対価としては
...
**この判定から出た、バックエンドと無関係な作業 2 件**（別単位として起票する。
本単位では実装しない）:

1. **ディスパッチを 1 コンピュートパスに畳む。** `compute.rs` の
   `ComputePipeline::dispatch` が `begin_compute_pass` をディスパッチごとに
   呼んでいる。再生形で 149.5 µs / 評価。バリアの意味は変わらない
   （wgpu はパス内のディスパッチ間でも依存を追う）が、
   ゴールデンテストでの確認が必要。
2. **ブロッキング読み戻しの 1 ms 切り上げを回収する。** `wait_timeout` を
   使った有界スピン → ブロック、または上流修正。読み戻し 1 回あたり約 1.32 ms
   で、アプリは公開フレームごとに 1 回払っている
   （`eval_hooks.rs` の `finalize`）。
