...
解像度上限の撤廃（`VRES-1` ✅ #300 で満たされた）か、4K 60 fps スクラブの
要求が立ったとき。前者はもう起きているので、**次に測るときはこの単位を
一緒に見直す**こと。
...
**ゼロコピー表示 — 未実装、引受先が無い。** `GPUBK-9`（✅ #296）は判断の
単位で、`gpu-backend-plan.md` の非対象節が「ゼロコピー表示の実装。
`GPUBK-9` で判断し、必要なら別計画」と書いている。**その別計画はまだ書かれて
いない。** これが `HIGH-09` の残りで、前提として `MED-GPU-07`（`Cargo.lock` に
wgpu が 2 本入っていて GPUI の device を受け取れない）の解消と、GPUI フォークの
範囲決めが要る。
