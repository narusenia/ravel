...
- **ゼロコピー Viewer 表示**（HIGH-09 の「本質」側）。**引受先は
  `zero-copy-viewer-plan.md`**（`GPUCOMP-11` で判断済み）。macOS は完了で、
  「GPUI は自前の wgpu デバイスを持つ」という上の前提自体が macOS では誤り
  だった — `gpui_macos` は wgpu ではなく Metal ネイティブで、同じ `MTLDevice`
  の上で interop する形に落ちた。
...
| GPUCOMP-11 | `VIEWER_MAX_DIM` の引き上げ / ゼロコピー表示の判断 | HIGH-09 | 判断は `GPUBK-9`（#296）で完了。上限は `VRES-1`（#300）が撤去済み。**ゼロコピー表示は `zero-copy-viewer-plan.md` が引き受け、macOS は完了（#382 / #384 / #386）。Linux は `ZC-8`、Windows は `ZC-7`** |
...
### GPUCOMP-11 ゼロコピー表示（判断は済み、実装は `zero-copy-viewer-plan.md` へ）
...
超えたので凍結しない）、`ZC-2`（#382）が GPUI の Metal デバイスを取り込み、
`ZC-3`（#384）が surface 経路を通し、`ZC-4`（#386）が寿命を閉じて既定を
有効にした。**macOS のビューアはリードバックを 1 回も行わない。**
Linux は描画側だけが入っており（#388）配線が `ZC-8`、Windows は `ZC-7`。
