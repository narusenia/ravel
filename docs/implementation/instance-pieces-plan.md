...
**上限は深さしか決めない。配置の順序も決める必要がある。** 内側のインスタンスの
配置を**先に**適用し、その結果へ外側の配置を掛ける（内 → 外）。深さの上限を
守ったまま向きや位置が狂う実装があり得るので、順序を明記する。式は
`InstanceTransform` の 1 本だけを使い（`TYPE-5` がラスタライザから集約した
もの）、合成の順序は `expand_instances` の入れ子処理が既に答えを持っている
ので**そちらに合わせる**（新しい合成関数を作るのではなく、既にある順序に
従う）。受入条件に**回転と拡縮が恒等でない入れ子のテスト**を置く。
...
- **ピースは自分の出自を持つ。** 元のインスタンス 1 行分の属性
  （`P` を除いた非配置属性 — `char_index` / `char_progress` / ユーザーが
  `attribute.*` で付けた任意の列）を、そのピースの **1 行の Instance ドメイン**
  として持たせる。これが無いと `PIECE-3`（属性を出力へ写す）に写す元が無い。
  `expand_instances` が既に同じ契約を持っている — あちらは元のインスタンス行を
  属性アキュムレータへ渡して生成した点・プリミティブへ配る。
  **入れ子でも出自を保つ**（内側のピースの行が外側の行に上書きされない）
- **画像ソースのインスタンスもピースになる。** ここは
  `expand_instances` と**意図的に違う**: あちらは画像ソースを警告して飛ばす
  （`ops.rs:1918`「輪郭に変換できない」）が、ピースは平らにするのではなく
  配る先を選ぶだけで、ラスタライザは画像インスタンスをそのままスタンプできる。
  画像を配れないと `geometry.from_image` の出力や画像を撒いた `scatter.*` の
  出力をピースにできず、機能の半分が落ちる
...
- **各ピースが元のインスタンス行の非配置属性を持っている**
  （`char_index` / 任意の列。`P` は持たない）
- **画像ソースのインスタンスがピースになる**（飛ばされない）
- 入れ子のピースで、内側の配置が先に効いた結果になる
  （回転・拡縮が恒等でないケース）
...
- **旧プロジェクトのフォールバックはプロセッサ側で決まる。**
  `Node::parameters` はそのまま保存されるので、`piece_mode` が存在する前に
  作られた scatter ノードは**そのエントリを持たずにロードされる**。
  テンプレートの既定が効くのは `create_node` が作る**新しい**ノードだけで、
  ロードされたノードには効かない（`ResolvedParams::str_or` は
  **呼び出し側が渡した既定**へ落ちる。`eval.rs:729`）。なので
  `params.str_or("piece_mode", "whole")` と書くこと。移行パスは要らない
- **`"instances"` のときは繋がっているワイヤの本数を問わず、全ワイヤを
  それぞれ分解して 1 本のピース列に平らにする**（ワイヤ 2 本 × 文字 5 =
  10 ピース）。ワイヤの順 → その中のインスタンスの順で並べる。
  「ピースを配る」という 1 つの説明で済み、選んだのに効かない制御が
  生まれない（UX 不変条件 6）
...
- **`piece_mode` は `source_mode` と別の軸**であることのテスト:
  ...
  効く
- **`piece_mode` を持たないノード**（旧プロジェクト）が `"whole"` として
  読まれる。保存 → 再ロードで値が変わらない
...
2. **`"instances"` は繋がっている全ワイヤを分解して 1 本のピース列に平らにする。**
   モードが 1 本のときだけ効く形（選んだのに効かない制御が生まれる）と、
   2 本以上をエラーにする形は却下した
