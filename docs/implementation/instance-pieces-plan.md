...
```text
scatter.* のパラメータ（名前は下記「判断」）:
  "whole"（既定 = 今の挙動） | "instances"
```
...
- 4 つの `scatter.*` テンプレート（grid / ほか 3 つ。実装時に
  `registry/builtin.rs` で数を確認すること）にモードパラメータを足し、
  `with_param_options` で `["whole", "instances"]` を宣言する
- `attach_instance_sources` の `[source]` の腕で、モードが `"instances"` かつ
  ソースがインスタンスを持つときだけ単位 1 で分解し、**既存の複数ソースの腕へ
  合流させる**。`source_mode` / `source_seed` の意味は変えない
...
- **2 本以上のワイヤを繋いだときの挙動が変わらない**（モードは 1 本のときだけ
  意味を持つ。2 本以上で `"instances"` を選んだときどうなるかは「判断」参照）
...
1. **モードのパラメータ名と値の名前。** 候補: `source_pieces`（`whole` /
   `instances`）、`piece_mode`、`split_source`。Houdini の語彙では「piece」。
   既存のモード列挙は `mode` / `<thing>_mode` という綴りが多い
   （`field.time` の `mode`、`scatter` の `source_mode`、`text` の
   `writing_mode`）ので `piece_mode` が規約には最も近い。ただし
   **`source_mode` が既にある**ので `source_pieces` / `piece_mode` の
   どちらでも「2 つの似た名前」になる — 読み手が取り違えないか
2. **2 本以上のワイヤ + `"instances"` のとき。** 取り得る形が 3 つ:
   - **(a) 各ワイヤをさらに分解して全部を 1 つのピース列に平らにする**
     （直感的。ワイヤ 2 本 × 文字 5 = 10 ピース）
   - (b) モードは 1 本のときだけ効き、2 本以上では無視する（今の挙動を保つが、
     選んだのに効かないのは UX 不変条件 6 に触る）
   - (c) エラーにする
   **(a) を推す。** 「ピースを配る」という 1 つの説明で済み、無視される控制が
   生まれない
3. **属性を写すのは `"instances"` のときだけか、`"whole"` でも写すか。**
   `"whole"` では「ソース全体」が 1 ピースなので、その Instance 属性を
   出力へ配る意味が薄い（どの文字の値を配るのか決まらない）。
   **`"instances"` のときだけ**を推す
