...
> 走査は `rasterize::flatten_geometry` と同じ形で、累積配置・継承した線幅・
> ルートの `join` を上から持ち回る。ストロークの張り出しは `stroke_margin` を
> コアへ移した `ops::stroke_reach(width, miter)` で、`rasterize` の被覆矩形と
> 同じ関数。**線幅は `stroke_width` 属性由来の分だけ**で、`rasterize` ノード
> パラメータの基底値はジオメトリから見えないので `LOW-APP-33` は残余として
> 開いている。
...
- **既定は `"fixed"`。** 既存プロジェクトの `shape.rect` は `sizing`
  パラメータを**持たずにロードされる**ので、プロセッサが
  `params.str_or("sizing", "fixed")` と書くこと。
  **テンプレートの既定が効くのは `create_node` が作る新しいノードだけ**で、
  ロードされたノードには効かない（`ResolvedParams::str_or` は呼び出し側が
  渡した既定へ落ちる。`crates/ravel-core/src/eval.rs:729`）。`auto` を既定にすると
...
- **フォーマット版の変更。** `sizing` は**新しいパラメータの追加**で、
  ...
  持たない文書はそのまま読める（`param_fold.rs` が「v4 文書の `center_x` は
  そのまま deserialize されて読まれなくなるだけ」と説明しているのと同じ性質）。
  **「持たない」をどう読むかを決めるのはプロセッサの `str_or` の既定**で、
  それを `"fixed"` にすることで既存シェイプが化けない（上記 単位 1）
