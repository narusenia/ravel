...
Document
 ...
 │                 ├─ In   (base_geometry, t, f, カスタムパラメータ)
...
- **In / Out ノード**: 動的ポート（カスタムパラメータ、カスタム出力
  ...
  `base_geometry` / `t`（`f` は後続の node-expansion-plan で追加）
...
**ステータス**: Done（実装単位 8-9）。計画からの逸脱: 「選択ノードの
単独プレビュー」は実装後にユーザー判断で削除し、Viewer は常に root comp
出力を表示する（単独プレビューが必要になれば display flag（v2）と併せて
再検討）。
...
- **Viewer の切替**: 「選択ノードの評価」から root comp 出力の常時評価
  へ（EvalService の要求対象変更）。**PlaybackController の評価要求も
  Document-aware evaluator の root comp 出力へ更新する**（後述の
  「関連計画との依存」を参照）。選択ノードの単独プレビューは実装後に
  削除（上記ステータス参照）
...
**ステータス**: Done（実装単位 10-11）。タイムラインのプロパティツリーが
キーフレームを持つネットワーク内パラメータを列挙し、ダイヤの追加
（チャンネル行ダブルクリック）・移動（ドラッグ）・削除（選択して
Delete）が Document 単位 undo で動作する。プロパティパネルの ◆/◇
トグルで殻 Transform/Opacity・In カスタムパラメータ・ノード
パラメータの現在フレームキー打ちが可能（定数 Float は Channel 化）。
dev プロファイルの compute クレート最適化で再生/スクラブの追従性を
改善（opt-level 0 のピクセルパスが ~1s/eval だった問題）。永続化は
`.ravprj` v3: Document 全体（Composition + 入れ子ネットワーク +
予約フィールド + media_assets）を document/main.ron に決定的 RON で
保存し、File メニュー（New/Open/Save/Save As）から読み書きできる。
v2 以前からのマイグレーション、ロード時の ID カウンタ前進
（REQ-LAYER-009）、bincode ジャーナルの版管理（旧形式は破棄）を含む。
サブネット内部ノードのタイムライン列挙は v1 制限（ui-impl-status.md
参照）。未保存変更ガード・自動保存は別計画。
