...
**ステータス**: 実装単位 10（アニメーション UI）Done。タイムラインの
プロパティツリーがキーフレームを持つネットワーク内パラメータを列挙し、
ダイヤの追加（チャンネル行ダブルクリック）・移動（ドラッグ）・削除
（選択して Delete）が Document 単位 undo で動作する。プロパティパネルの
◆/◇ トグルで殻 Transform/Opacity・In カスタムパラメータ・ノード
パラメータの現在フレームキー打ちが可能（定数 Float は Channel 化）。
併せて、dev プロファイルの compute クレート最適化で再生/スクラブの
追従性を改善（opt-level 0 のピクセルパスが ~1s/eval だった問題）。
残り: 実装単位 11（永続化）。
