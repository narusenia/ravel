...
> **本計画は `done/exposed-parameters-plan.md`（REQ-PROJ-006）の前提**
> （2026-08-03 追記）。公開パラメータ宣言は CLI の `--param`・サブグラフ
> テンプレート・ネットワークインターフェース・シェーダ manifest の共通機構だが、
> その束縛先となるポートモデルを編集する経路が本計画で入る。ポートを生やせない
> 状態で宣言の層を設計すると二度手間になるので、**順序は本計画が先**。
...
結果として、**カスタムポートはテストフィクスチャと `crates/ravel-project/src/lib.rs`
のデモデータにしか存在できない**。
...
`done/exposed-parameters-plan.md` の `EXPO-2` が束縛を「ノード ID + パラメータキー」で
...
### 単位 4: ポート右クリック（ノードエディタ） — 実装済み（#267）
...
- **ピンは境界エッジ 1 本ごとではなく、外側の端点ごとに 1 つ**。入力ピンは
  横切るエッジが出ていく `(ノード, 出力ポート)`、出力ピンは選択内側の同じ組で
  束ねる。入力側の端点で束ねると、1 つの出力が 3 ノードを駆動しているだけで
  ピンが 3 つ生え、外側の配線本数が変わる — それは入れ子にしたのではなく
  別のグラフ。束ねれば「外側の 1 本は 1 本のまま」で、内側で再び分岐する
- **ピン名は境界エッジの入力ポート名**（入力ピンなら移動したノード側、
  出力ピンなら外側の読み手側）。「その値が何に使われるか」を言うのは入力側で、
  出力側でもこの規則がそのまま良い結果になる: `net.out` へ向かう選択を
  まとめると出力ピンが `frame` になり、手で作ったサブネットと同じ形に着地する。
  同じ側で既に使われている名前（In の予約名を含む）は `_2` / `_3` … を付ける
- **`frame` の補完は「何も出ていかない選択」に限る**。`new_subnet_inner_graph`
  が `frame` を持つのは出力ピン 0 個のノードが評価できないため。横切るエッジが
  あるならそれがピンなので、余分な `frame` を足すと使われないピンが残る
- **選択を出て戻ってくる経路は拒否する**（`CollapseWouldCycle`）。
  subnet ノード 1 個を経由する自己ループになり `Graph::add_edge` が撥ねる。
  メニューは `can_collapse` で先に無効化するので、実行時に届くのは
  メニューが開いている間にグラフが動いた場合だけ
- **ノード ID は移動しても変えない**。ID は文書全体で一意（`Evaluator` の
  プロセッサ表は平坦な `HashMap<NodeId, _>`）なので、階層を 1 つ下げても
  衝突しない。内側どうしのエッジも ID を保つ。作り直すのは境界を張り替える
  エッジだけで、**collapse → extract のラウンドトリップでエッジ ID は戻らない**
  （1 本が 2 本になり、また 1 本に戻るので対応が決まらない）。テストは
  接続関係（ノード ID + ポート名の組）で比較する
- **新規 3 ノードの ID はグラフに在る ID を避けて採る**。`NodeId::next()` は
  ロード後なら文書のどの ID より先だが、明示 ID で組んだグラフ（テスト、
  フィクスチャ）はそうと限らない。`seed_subnet_node` が owner ID を避けるのと
  同じ理由・同じ形
- **extract は内部 In の固定ポートを親の In へ繋ぎ直す**。`t`（あれば `f` /
  `base_geometry` / `source`）はピンではなく `EvalContext` 由来なので、
  親の同名ポートは同じ値。繋ぎ直さないと展開したノードが時間を失う。
  一方、**外側が未接続のカスタムピン**は promote パラメータが値を供給していて
  親グラフにその置き場が無いため、素直に未接続へ落とす（移動したノード自身の
  パラメータへフォールバックする）
- **extract は親と ID が衝突したノードだけ採番し直す**。文書が作ったグラフでは
  起こらないが、手組みのグラフでは起こる。`ChannelSource::NodeOutput` の
  追従には `graph.rs` の `remap_parameter_node_outputs` を `pub(crate)` にして
  共有した。**採番し直しは移動するノード自身までで、その内部グラフには
  及ばない**
- 位置は collapse が subnet ノードを選択の重心へ、extract が展開後の重心を
  subnet ノードの位置へ戻す。差し引き 0 なのでラウンドトリップで位置も一致する
- **選択は collapse 後に subnet ノードへ、extract 後に出てきたノード群へ移す**。
  extract 側は「今在って前に無かった ID」で求める — 採番し直しがあるので
  内部 ID をそのまま使えない
- コンテキストメニューは Delete / Bypass と同じく `targets`
  （選択、無ければ右クリック先）に対して働き、パネルのメソッドを直接呼ぶ。
  Action ハンドラ（`CommandId` 経路）は選択に対して働く。実体は 1 つの
  メソッドで、どちらも Document コミット 1 回
- **失敗時の UI 通知は入れていない**。メニューが無効化で先に潰すので、
