...
> （2026-07-29 に Phase 0 を 0a / 0b に分割。2026-08-06 に `PATH-0b` を調査し
> **推奨: `earcut` クレート（ユーザー承認待ち）**。`PATH-0a` は未決）
...
**推奨: `earcut` 0.4.11（production 依存の追加。ユーザー承認待ち）**
...
承認が下りなかった場合の代替は自前実装だが、そのとき **FRAC-1 の規模見積りを
300 行から改める**必要がある。実装時の申し送りとして、`hole_indices` の単調性は
呼び出し側で検証し、破れていたら panic させずに `GeometryError` に落とすこと。
