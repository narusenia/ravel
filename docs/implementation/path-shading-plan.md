...
per-pixel 評価器そのものは上のとおりで、**塗りは案 A とする**。線だけ頂点色を
補間し、塗りは従来どおり `Cd` のプリミティブ値 1 色のままにする。

### 案 A: 線だけ。塗りはプリミティブ色のまま（採用）

- 最近傍セグメント + `t` で線の色を補間する。塗りは今までどおり
  `Cd` のプリミティブ値 1 色
- **ロードマップの約束（線に沿ったグラデーション）をそのまま満たす**
- 塗りに Point ドメインの `Cd` を書いた場合は**無視する**。現状も塗りは
  プリミティブ値 1 色で描いているため挙動が変わらず、平均や先頭を採ると
  既存の絵が黙って変わる。この扱いは `PSHADE-5` の文書更新で利用者向けに
  明記する
- 実装は CPU / GPU とも小さい。新しい依存なし

案 B / C は採用しない。理由と将来の再検討余地は「やらないこと / 見送る選択肢」
に記載する。
...
位置由来（この単位）とトポロジ由来（`PSHADE-2`）は独立した
仕組みで、両方の指定が同時に存在しうる。
...
- **案 B: 塗りも最近傍境界セグメントで補間すること。** 塗りの頂点色は
  ロードマップが要求している線のグラデーションの範囲外であり、星形や凹多角形
  では領域が割れて見えるため見送る
- **案 C: 塗りを三角形分割して重心座標で補間すること。** 期待どおりの Gouraud
  塗りになるが、現在の `rasterize` には三角形描画経路が無く、メッシュを明示的に
  拒否しているため見送る（`ensure_planar_paths`、
  `crates/ravel-nodes/src/rasterize/mod.rs`。テスト
  `mesh_primitives_are_an_explicit_error` が pin している）。ロードマップの要求は
  線であるため、今はこの経路を追加しない。**足りないのは分割器ではなく描画経路**
  で、`crates/ravel-core/src/geometry/triangulate.rs` の `Triangulator`
  （`PATH-0b`、#301）と `Primitive::Mesh` は既にある。将来 `3D-4` の三角形
  レンダラが入ったら再検討できる
...
- **メッシュプリミティブの描画そのもの**。一般的な三角形描画は
  `3d-scene-plan.md` の `3D-4` の担当
