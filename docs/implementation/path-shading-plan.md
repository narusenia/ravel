...
- **三角形分割器が要る**（耳刈り / 制約付き Delaunay）。凹多角形・自己交差・
  穴に耐える実装は小さくない。**本番依存の追加**か、相当量の自前コード
- GPU 側も三角形を送る形に変わるので、`DrawItem` のバッファ構造が変わる
- 被覆は zeno のまま、色だけ三角形から引く、という形にはできる

**推奨は A**。理由は基準どおり — ロードマップが要求しているのは線で、塗りの
グラデーションは `style-attributes-plan.md` の「非対象」に明記がある
（「1 要素内のグラデーション塗り / パターン塗り」）。C が要るとわかってから
C をやる方が、要らない三角形分割器を抱えるより安い。

**この判断は保留する。** 単位 1 に入る前に決める。
