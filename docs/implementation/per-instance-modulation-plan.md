...
`geometry-pipeline-ui-plan.md`（#60–#63）で完了済み。本計画はその上に載る
...
**実測ベースライン**: `perf-baseline.md` シナリオ (c) で
`shape.rect → scatter.grid(500)` のジオメトリ評価は**温キャッシュで
0.007 ms**。フレーム予算の 2000 分の 1 で、現時点で最適化対象ではない。
本計画は **CPU（rayon）で実装する**。
