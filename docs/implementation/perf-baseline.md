...
`ravel-core` の `--release` ビルドで、1920×1080 相当（2,073,600 画素 ×
RGBA）を 1 パス。量子化のみ（`quantize_u8` を 4 回）と、変換込み
（`to_display_rgba8`）を同一データで交互に測った。出力バイト列は両者の
比較用アキュムレータで同一を確認している。

| 経路 | 1920×1080 1 フレーム |
|---|---|
| 量子化のみ（CM 以前の Viewer 相当） | **0.73 ms** |
| 変換 + 量子化（現行） | **34.6 ms** |
| 参考: 伝達関数を `f32` の `powf` で評価した場合 | 24.9 ms |

`ViewerImage::from_frame_buffer` 全体（BGRA 並べ替えと `RenderImage` 生成を
含む）では 1920×1080 で 47.0 ms、1024×576 で 13.8 ms。

### 読み方

- **UI スレッドではない。** `GPUCOMP-9` 以降この変換は評価ワーカーで走るので
  操作が固まることはなく、効くのはプレビューのスループット。Viewer は
  `ViewerResolution` の対話用上限で評価するので、実効値は 1024×576 側に近い
- 支配項は色成分ごとの `powf`。`f32` 版は 28% 速いが定数をもう 1 組持つ対価に
  見合わないと判断した（`color.rs` の doc コメントに理由を残してある）
- **正しい直し方は `CM-7`**。変換を GPU の 3D LUT に焼いて wgpu 経路で適用
  すれば、このコストは削れるのではなく無くなる
- 書き出し側も同じ変換を 1 フレームにつき 1 回払う（`to_output_space`）。
  EXR は作業空間のままなので**払わない**

### 測定条件（そのまま読まないこと）

- loadavg 2.9〜6.3（1 分〜5 分平均）、M 系ラップトップ、`--release`。同じ機体で
  別ワークツリーのビルドが並行しているので、**絶対値ではなく 2 経路の比**を
  見ること。交互に測っているので比は頑健
- 1 パスのみでウォームアップを取っていない。桁の主張（0.7 ms 対 35 ms）に
  必要な精度は満たすが、数 % の差を論じる用途には足りない
