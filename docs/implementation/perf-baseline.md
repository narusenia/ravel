...
#### 再計測（2026-08-12、`MED-GPU-04` 対応後）

同ハーネス・同シナリオ。ビューアの経路が GPU ラスタライザへ移り
（`GpuEvalHooks::finalize` がコンテキストとプールを持っているのに毎フレーム
CPU プロセッサを構築していた）、CPU 参照経路もカバレッジマスクを 1 枚再利用し
プリミティブの bbox に限定するようになった。同一 loadavg 下の測定。

| 経路 | rasterize span | 対 baseline |
|------|----------------|-------------|
| CPU 参照（対応前） | mean 39.58 ms | — |
| CPU 参照（bbox + マスク再利用後） | mean 2.28 ms | 17.4× 短縮 |
| GPU（現ビューア経路） | mean 0.111 ms | 356× 短縮 |

ハーネスは比較を揃えるため GPU 経路でも毎フレーム読み戻す（`gpu_readback`
mean 1.94 ms）。表示変換を持つ実ビューアはこの読み戻しをせず、常駐フレームを
そのまま表示変換へ渡す。
...
## プレビュー解像度の係数ごと（`VRES-5`）

計測日: 2026-08-24。環境: Apple M5 / macOS 26.3 / release ビルド /
10 レイヤーのシェルチェーン / `perf_baseline` の「Viewer path at display
resolutions」節。各 20 フレーム、4 ラウンド。**この機体の loadavg 平常値は
3 前後**なので絶対値ではなく係数間の比を読む（ラウンド開始時の loadavg:
5.99 / 5.73 / 4.76 / 3.71）。

`VRES-2`〜`VRES-4` で係数が選べて永続化されて適応で下がるようになったので、
**係数ごとに実寸で測り直した**。16:9 1080p コンポジションを
`ViewerResolution::apply` に通した寸法がそのまま行になる。
1024x576 は撤去した旧上限の寸法で、過去のラウンドと突き合わせるための錨。

| 係数 | 解像度 | MB/frame | mean ms（4 ラウンド） | 中央値 | 読み戻し mean ms |
|---|---|---|---|---|---|
| — （旧上限の錨） | 1024x576 | 9.4 | 4.14 / — / — / 4.15 | **4.15** | 3.17〜3.21 |
| `Full` | 1920x1080 | 33.2 | 12.65 / 12.65 / 12.66 / 12.40 | **12.65** | 9.77〜9.95 |
| `Half`（既定） | 960x540 | 8.3 | 3.73 / 4.00 / 3.76 / 3.69 | **3.74** | 2.91〜3.03 |
| `Quarter` | 480x270 | 2.1 | 1.84 / 1.94 / 1.81 / 1.86 | **1.85** | 1.53〜1.59 |

**係数間の比。** `Full` は `Half` の **3.4 倍**、`Half` は `Quarter` の
**2.0 倍**。面積比は 4 倍と 4 倍なので、**どちらも面積比より緩い** —
1 フレームには解像度に比例しない固定費（サブミット、パイプライン状態、
評価のノード走査）が乗っており、小さいほどその割合が大きい。

この 3 つの数字が計画の判断を裏づける:

- **既定を `Half` にしたのは妥当。** 3.74 ms は 60 fps の 16.7 ms 予算に
  対話操作の余地を残す。`Full` の 12.65 ms は予算のほぼ全部を 1 フレームの
  評価と読み戻しに使う
- **`Full` は「止めて確認する」用途で使える。** 12.65 ms は静止画の確認には
  十分速く、`CM-7` 前の見積り（転送 5.54 + CPU 変換 6.44 = 出口だけで
  12 ms）が消えた分がそのまま効いている。**4K で `Full` を常用する話は
  別**（この表は 1080p）
- **適応で 1 段落とす効果は `Full` 選択時に最大。** ドラッグ中の 1 フレームが
  12.65 → 3.74 ms（**3.4 倍速い**）。`Half` 選択時でも 3.74 → 1.85 ms
- **`Quarter` から更に落とさない判断も妥当。** 1.85 ms は既に固定費が支配
  する領域で、更に半分にしても取り戻せるのは 1 ms 未満

読み戻しが各行の大半を占める（`Full` 77〜79%、`Half` 78〜81%、
`Quarter` 83〜86%）。**係数を下げるほど読み戻しの割合は上がる** — 面積が
減ると評価側（1.85 ms の行では約 0.29 ms）の方が速く縮むので、小さい係数では
残るコストがほぼ転送になる。ゼロコピー表示（`zero-copy-viewer-plan.md`、
macOS は完了）はこの表の経路（`to_frame_buffer` を通す計測用の経路）ではなく
実アプリの表示経路に効く。
...
同じ 3 シナリオを同じテストで測り直した値。シナリオ定義は 1 つ変えた —
再生は「静的なレイヤーを選択」と「不透明度をキーフレーム化したレイヤーを選択」の
2 本に分けた。前者が削減対象、後者が**削ってはいけない**方である。

| シナリオ | Properties | Timeline | Outliner | MediaBin |
|---|---|---|---|---|
| ドラッグ 10 move | 16 → **10** | 10 | 10 | 10 |
| 再生 30 フレーム（静的レイヤー） | 30 → **1** | 0 | 0 | 0 |
| 再生 30 フレーム（アニメーション有り） | 30（据え置き） | 0 | 0 | 0 |
| コンポジション切替 1 回 | 0 | 2 → **1** | 2 → **1** | 1 |

- ドラッグは **move あたりちょうど 1 回**。ドキュメントは毎 move 実際に変わるので
  1 が下限であり、Properties の 16 は `MED-UI-06` の二重再解決だった。
  グローバル駆動の sync が epoch を記録し、対になる project notify が
  それを見て飛ばす（`RESP3-11`）。
  なお計測は**ドラッグ 2 move 分のウォームアップ後**に取る。ターゲットが
  Empty から切り替わる最初の 2 move は「新しいターゲットを採用する」分岐で、
  値の解決を `render` の `rebuild_widgets` に任せる（ヘッドレスのテストは
  描画しないので 0 と数えられる）。定常状態が測りたいものである。
- 再生は、表示中のものが**どれもプレイヘッドで再サンプルされない**とき
  1 回で止まる（最初の 1 フレームで判定して以後スキップ）。
  キーフレームや式や被駆動パラメータがあれば 30 回のまま — こちらを削ると
  再生中に値が止まって見える。
- コンポジション切替は 1 回。epoch ではなく**そのパネル自身が今どの
  コンポジションを映しているか**で判定しているので、project notify を
  伴わないグローバル書き込みでも鏡は追従する
  （`a_bare_composition_global_write_still_moves_the_mirrors`）。

### 測っただけで、まだ削っていないもの

- **MediaBin はドラッグ 1 move ごとに同じ行を作り直す**（上表の 10）。
  レイヤー編集は media 資産を変えないので、行が同一なら早期 return できる。
  ただしそうすると `HIGH-07` の既存テスト 2 本
  （`a_completed_save_rebuilds_no_document_panel` /
  `a_composition_switch_leaves_every_gate_open_for_the_next_edit`）が落ちる。
  両者は「ドキュメント編集では**すべての**ミラーパネルが notify する」と
  主張しており、このパネルに関してはその前提の方が誤りだが、`HIGH-07` の
  退行テストを緩めるのは性能作業のついでに決めることではない。
- **Timeline の行レイアウト走査は全レイヤー分残っている（意図的）。**
  カリングが届くのはヘッダのウィジェットサブツリーと painter の描画である。
  「可視行がどこから始まるか」は手前の行の高さの合計なので、
  `layer_blocks()` の走査自体は全レイヤーを回る必要がある。
  ただし `visible_property_rows()` は**展開チェックの内側**にあるので、
  折り畳まれたレイヤー 1 枚のコストは展開状態の HashSet 参照 1 回と
  16 バイトの push 1 回で、行モデルの構築もアロケーションも無い。
  **100 レイヤーが全部折り畳まれていれば行モデルの構築は 0 回**
  （`the_row_layout_walk_builds_row_models_only_for_expanded_layers` が固定）。
  残るコストはスタックの深さではなく**展開済みレイヤー数**に比例し、
  展開はレイヤー 1 枚ずつの明示操作である。
  レイヤーごとの高さをキャッシュすれば消せるが、無効化条件
  （ドキュメント epoch・レイヤー展開・プロパティ展開・チャンネル数）を
  1 つ外すと、古い高さより下の行が**全部ずれた位置に描かれる**。
  性能のために表示位置の正しさを賭ける取引になるので採らなかった。
- **`MED-UI-05` の評価更新経路は既に無い。** 再生 30 フレームで
  Timeline / Outliner / MediaBin はいずれも 0 回。`CRIT-01` が
...
## テキストレイアウト baseline（`typography-plan.md` 単位 2 の判断ゲート）

計画: `typography-plan.md` 単位 2 の完了条件。計測日: 2026-09-04。
環境: Apple M5 / macOS 26.3 / release ビルド。
ハーネス: `crates/ravel-core/examples/text_layout_baseline.rs`
（`cargo run -p ravel-core --release --example text_layout_baseline`）。

**全行を未キャッシュで測っている。** グリフキャッシュは存在しないので、
`layout_text` は毎回フェイスを parse し、全段落をシェイプし、異なる
クラスタごとに輪郭を取り直す。単位 2 の時点でありうる最悪値である。

段の内訳は `text::LayoutTiming` が返す（`layout_text_timed`）。
`shape` は rustybuzz、`glyph` は輪郭抽出（**異なるクラスタごとに 1 回**）、
`place` は残り（行分割・整列・アトリビュート列の書き出し）。3 段は
呼び出し全体に必ず一致する — `place` を差分で出しているため。

パラメータ: `size = 72`、`wrap_width = 1600`（折り返しを毎回通す）、
`tracking = 0`、`leading = 0`（フェイス自身の行送り）、`align = left`。

### 結果（ms、9 ラウンドの中央値）

| フェイス | 文字数 | total | shape | glyph | place | インスタンス | 異なるグリフ |
|---|---|---|---|---|---|---|---|
| Geist Regular（ラテン散文） | 100 | 0.064 | 0.025 | 0.022 | 0.016 | 98 | 29 |
| Geist Regular（ラテン散文） | 1000 | **0.283** | 0.194 | 0.022 | 0.059 | 979 | 29 |
| Noto Sans JP（日本語散文） | 100 | 0.065 | 0.014 | 0.030 | 0.020 | 100 | 26 |
| Noto Sans JP（日本語散文） | 1000 | **0.204** | 0.107 | 0.029 | 0.064 | 1000 | 26 |

インスタンス数が文字数より少ない行（98 / 979）は、行末に来た空白が
インスタンスにならないため（`layout_text` の規約）。

### 16.6 ms（60 fps 予算）に対する位置

1000 文字のレイアウト 1 回が **0.28 ms = 予算の 1.7%**。予算を使い切るには
同時に **約 58 個**のテキストノードを毎フレーム再レイアウトする必要がある。

**判断: グリフキャッシュは単位 2 では入れない。** 計画が
「16.6 ms を超えるならグリフキャッシュの設計を見直す」と書いた条件に
2 桁足りない。入れるとしたら根拠は総コストではなく、下の「輪郭は
アルファベットに比例する」の性質になる。

### 輪郭抽出は文字数ではなくアルファベットに比例する

`glyph` の列が 100 文字と 1000 文字で動いていない（0.022 / 0.022、
0.030 / 0.029）。`instance_sources` を異なるクラスタごとに 1 本しか作らない
設計の直接の結果で、10 倍の文字数でも輪郭抽出の仕事は増えない。

つまり**輪郭のキャッシュが効く相手は「長い文字列」ではなく「多くの
テキストノードが同じフェイスを使う状況」**である。単位 2 の時点では
ノード 1 個あたり 0.03 ms なので、キャッシュを持つ理由はまだ無い。

### 支配的なのはシェイピング

1000 文字で `shape` が total の 69%（ラテン）/ 52%（日本語）。
ラテンが日本語より重いのは合字・カーニングの GSUB / GPOS 参照があるため
（同じ 1000 文字で 0.194 ms 対 0.107 ms）。

シェイピングは段落単位でしか区切れない（`wrap_paragraph` は 1 回の
シェイプ結果から行を切り出す）ので、ここを削るなら手は
「段落単位のシェイプ結果のメモ化」で、`text` パラメータが変わらない
フレームでは丸ごと再利用できる。**文字ごとの変調（フィールド）は
`text` を変えないので、リリックモーションの再生はこの 0.28 ms を
毎フレーム払う必要が無い** — 払っているのは現状の実装であり、
必要になったら段落メモ化を入れる。

### この数字で言えないこと

- **`text.to_path`（単位 5）と `text.on_path`（単位 4）を含まない。**
  輪郭点をポイントドメインに展開するコストは別に測る
- **ラスタライズを含まない。** 表に出ているのは Geometry を作るまで。
  文字数分のインスタンス描画は `rasterize` 側のコストで、
  上の「ジオメトリ評価スケーリング」節の instance 経路と同種
- **フェイスの parse を毎回含む。** `FontRef` のバイト列は
  `FontLibrary` が共有しているが、`rustybuzz::Face::from_slice` は
  レイアウトごとに走る。4.5 MB の CJK フェイスでもこれが
  支配的でないことは上の日本語行が示している（total 0.204 ms）
- 計測時の loadavg は 7 前後（デスクトップアプリ由来、ビルドは停止）。
  5 回の実行で 1000 文字 total は 0.256〜0.320 ms に収まった。
  絶対値ではなく**段の比と 16.6 ms との桁差**が結論の根拠である
