...
**loadavg 2.72**（1 分値。ハーネスが開始時に出力した値で、3 回とも
`2.72 2.79 3.26`。実行直前の `uptime` は 2.26〜2.72）。上の `CM-7` 節と違い、
この計測は他のビルドやベンチが走っていない静穏時のもの。
1 セルにつき 20 ラウンド、各ラウンドで 3 つの計測を**交互に**取り、
その平均。**3 回実行**。
...
ゼロコピーの得の下界**として扱う。段 4 は下界に上積みされるだけなので、
未測定でも判断の向きは変わらない。

### 結果（3 回分を `/` 区切り、単位 ms）

| 合成解像度 | factor | 実寸 | 段 1+2 | 段 2 | 段 3 | **下界（2+3）** |
|---|---|---|---|---|---|---|
| 1920×1080 | Full | 1920×1080 | 1.722 / 1.734 / 1.744 | 1.479 / 1.458 / 1.492 | 0.372 / 0.376 / 0.360 | **1.851 / 1.833 / 1.852** |
| 1920×1080 | Half | 960×540 | 1.336 / 1.357 / 1.346 | 1.332 / 1.328 / 1.336 | 0.093 / 0.091 / 0.102 | **1.425 / 1.419 / 1.439** |
| 1920×1080 | Quarter | 480×270 | 1.328 / 1.345 / 1.320 | 1.302 / 1.322 / 1.304 | 0.034 / 0.032 / 0.036 | **1.336 / 1.354 / 1.341** |
| 3840×2160 | Full | 3840×2160 | 4.040 / 4.047 / 3.563 | 2.103 / 2.349 / 1.989 | 1.401 / 1.470 / 1.307 | **3.504 / 3.819 / 3.295** |
| 3840×2160 | Half | 1920×1080 | 1.503 / 1.512 / 1.533 | 1.440 / 1.415 / 1.456 | 0.774 / 0.795 / 0.335 | **2.214 / 2.210 / 1.791** |
| 3840×2160 | Quarter | 960×540 | 1.341 / 1.309 / 1.330 | 1.337 / 1.317 / 1.312 | 0.123 / 0.084 / 0.106 | **1.460 / 1.402 / 1.418** |

転送回数はどのセルも **20 ラウンドで 40 リードバック / 0 アップロード**
（`run` の中で 1 回、段 2 の単独計測で 1 回）。入力フレームはラウンドが
始まる前に GPU へ上げてあるのでアップロードは 0。**負荷は時間を動かすが
回数は動かさない**ので、回数はこの表の妥当性のチェックに使える。

**段 2 は `run` が使うのとは別のテクスチャで単独計測している**（同じキー・
同じサイズだが同一のリードバックではない）。そのため「段 2 > 段 1+2」
（run 2 の 4K Quarter: 1.317 > 1.309）や「下界 > 段 1+2」（1080p Full の
全 run など、段 3 がディスパッチ 1 回分を上回るセル）が出る。
数字の整合性でつまずかないこと — 段 1 の値をこの表から引き算で出すのは
近似にしかならない。
...
**全 18 セルが下界だけでしきい値を超えている。**

- 最小セル（1080p Quarter）: 1.336〜1.354 ms = **1.60×**
- 最大セル（4K Full）: 3.295〜3.819 ms = **3.95〜4.57×**
- Viewer の既定に近い 1080p Full: 1.833〜1.852 ms = **2.20×**

**往復を消して得られる時間**は、1080p Quarter でも 1.3 ms 以上、
4K Full で 3.3〜3.8 ms。段 4（GPUI のアップロードとアトラス churn）は
未測定だが、**消える側にしか効かない**ので、これに上積みされることは
あっても差し引かれることはない。よって計画を凍結する条件
（得が 0.835 ms を下回る）は満たされず、**`ZC-2` 以降へ進む**。
