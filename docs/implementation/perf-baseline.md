...
実施」。**直列 18.24 ms（再実行 19.51 ms、min 16.82 / 17.14 でも超過）で
超過**。パイプライン化した 13.59 ms は 60fps 予算内に入るが、これは
...
**ステップ自体は安い。** 10 万点で 0.35〜0.41 ms、100 万点でも rayon で
1.01 ms。10k で rayon が serial より遅いのは、この規模では
スレッド起動コストが仕事量を上回るため。

**律速は描画経路**。10 万点で `flatten` 2.29 ms + `upload` 0.99 ms で、
ステップの 8 倍。インスタンス（D の 8.75 ms / 100k）より安いのは、
...
**サイズにほぼ依存しない ≈1.3 ms**。submit + map の固定レイテンシが支配的
（unified memory なので帯域は問題にならない）。GPU sim の状態を毎フレーム
CPU 側へ戻すコストは 16.6 ms 予算の約 8%。

### 判断: 単位 6（GPU sim）の根拠はステップコストではない

`particle-plan.md` は単位 6 の完了条件に「10 万点で 1 フレームの step が
16.6 ms を大きく下回ること」を置いているが、**CPU 側が既に 0.35 ms で
下回っている**。よって:

- **単位 1〜5 を CPU（rayon）で作る方針は実測で裏付けられた。** 10 万点なら
  CPU ステップで 60fps 予算の 2% しか使わない。
...
- 読み戻しが ≈1.3 ms で固定なので、「スクラブ中は CPU / 再生中は GPU」の
  使い分けは帯域面では成立する。VRAM とキャッシュの衝突（同計画の未解決節）
  は別問題として残る。
...
| 転送 | 実測 |
|---|---|
| アップロード（CPU → storage buffer） | 6.5〜7.9 GB/s |
| 読み戻し（buffer → CPU、16 MB） | ≈11 GB/s（固定 1.3 ms が支配） |

頂点 32 B（位置 + 法線 + UV 相当）換算:
...
3〜13%）。100 万頂点を毎フレーム上げると単一メッシュで予算の 27% を使う。
...
- 2 回実行の突き合わせのみ（D の 100k は 18.24 / 19.51 ms）。
