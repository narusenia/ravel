...
10 レイヤーのシェルチェーン + ビューアの読み戻し。
`cargo run -p ravel-nodes --release --example perf_baseline` の
「Viewer path at display resolutions」節。各解像度 20 フレーム、3 ラウンド。

`VIEWER_MAX_DIM = 1024`（`crates/ravel-app/src/project_state.rs`）は
「シェル合成が CPU でレイヤーごとに readback する」ことへの譲歩として入った。
**その前提は既に消えている**（`GPUCOMP-2/3/4/9`、`GPUBK-6`）ので、
上げられるかを測る。16:9 のコンポジションはこの上限で `1024x576` に落ちる。

| ラウンド | loadavg | 解像度 | mean ms | min ms | max ms | うち読み戻し ms | MB/frame | readbacks/frame |
|---|---|---|---|---|---|---|---|---|
| 1 | 1.99 | 1024x576 | 5.93 | 4.21 | 6.92 | 4.64 | 9.4 | 1.00 |
| 1 | 1.99 | 1920x1080 | 20.13 | 15.60 | 23.40 | 16.01 | 33.2 | 1.00 |
| 2 | 2.84 | 1024x576 | 5.52 | 4.12 | 6.92 | 4.37 | 9.4 | 1.00 |
| 2 | 2.84 | 1920x1080 | 19.10 | 15.30 | 21.85 | 15.00 | 33.2 | 1.00 |
| 3 | 3.34 | 1024x576 | 5.16 | 3.99 | 7.21 | 4.06 | 9.4 | 1.00 |
| 3 | 3.34 | 1920x1080 | 29.47 | 16.17 | **72.75** | 16.15 | 33.2 | 1.00 |

中央値: `1024x576` が **5.52 ms**（読み戻し 4.37 ms）、
`1920x1080` が **20.13 ms**（読み戻し 16.01 ms）。

**ラウンド 3 の 1080p の合計は汚れている。** 走行中に loadavg が 3.34 まで
上がり、max が 72.75 ms に伸びた。**読み戻しだけは 3 ラウンドで安定している**
（16.01 / 15.00 / 16.15）ので、伸びたのは評価側の CPU スケジューリングで
GPU 側ではない。判定にはラウンド 1 / 2 を使う。

### 判定: `VIEWER_MAX_DIM` は上げない。ゼロコピー表示を別計画に切る

1. **1080p は 60 fps 予算に入らない。** 20.13 ms > 16.7 ms。30 fps 予算
   33.3 ms なら入る（60%）が、対話中のスクラブが 50 fps を割る形になるので
   上限の撤廃としては成立しない。現行の `1024x576` は 5.52 ms = 60 fps 予算の
   33% で余裕がある。
2. **どちらの解像度でも読み戻しが約 80% を占める**（4.37 / 5.52 = 79%、
   16.01 / 20.13 = 80%）。**そして `readbacks/frame` は既に 1.00** —
   `GPUCOMP` の GPU 常駐化でレイヤーごとの読み戻しは消えており、
   **回数を減らす余地はもう無い**。
3. **読み戻しは帯域律速。** 9.4 → 33.2 MB/frame（3.5 倍）に対し
   4.37 → 16.01 ms（3.7 倍）。固定費ではなくバイト数で伸びるので、
   解像度を上げれば比例して増える。

したがって上限を上げるには**読み戻しそのものを無くすしかない** =
ゼロコピー表示（GPUI カスタム要素 / デバイス間 interop で、評価結果の
テクスチャを CPU を経由せずに表示へ渡す）。これは `GPUBK-9` の範囲を超える
実装なので、**別計画に切る**（`GPUCOMP-11` が「維持できないならボトルネックを
特定し、ゼロコピー表示を別計画に切る」と書いているとおり）。
`GPUBK-9` が固定したデバイス共有の契約（`interop::context_from_wgpu`、
`ravel-app` のみ）がその前提になる。

### この数字で言えないこと

- **測ったのは評価 + 読み戻しで、画面に出るまでではない。**
  GPUI 側のテクスチャアップロードと present は含まない。**実際の表示レートは
  実機でしか確認できない**ので、上の 20.13 ms は「ビューアが 1 フレームに
  払う下限」として読む。ゼロコピー表示の計画を立てる時点で実機の present
  コストを測る必要がある。
- ヘッドレスの `EvalService` 経路ではなく直接評価しているので、
  評価ワーカーのスレッド往復とキャッシュのヒット率は反映されていない。
- 10 レイヤー固定。レイヤー数を増やすと評価側が伸びるので読み戻しの比率は
  下がるが、読み戻しの絶対値は解像度だけで決まる。
- `GPUCOMP-10`（非同期リードバック）の再評価条件は「`VIEWER_MAX_DIM` を
  撤廃したとき」で、**今回は撤廃しないので条件は満たされない**。ただし
  ゼロコピー表示に踏み込む場合、読み戻しが 80% を占めるこの数字は
  `GPUCOMP-10` の前提も変えるので、その計画で読み直すこと。
