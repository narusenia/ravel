...
| 解像度 | フレーム | 変更前 `3e437db` | 変更後 (1) | 変更後 (2) | 変化 |
|---|---|---|---|---|---|
| 1920×1080 | 33.2 MB | **6.13 ms**（4.82–7.07） | **2.36 ms**（1.88–3.57） | **2.44 ms**（1.80–3.74） | **−61%** |
| 3840×2160 | 132.7 MB | **26.89 ms**（23.29–67.11） | **7.58 ms**（5.11–9.79） | **6.23 ms**（5.29–7.22） | **−72〜−77%** |

変更後は 2 回実行した。ばらつきは 1080p で ±0.08 ms、4K で ±1.35 ms。
...
ハーネスは同じ readback を `begin_readback()` + `is_complete()` で
分割しても測る。前半は GPU のコピー完了待ち（非同期化すれば次フレームの
評価と重ねられる分）、後半はマップ範囲から CPU バッファへのコピー
（重ねられない分）。

| 解像度 | GPU コピー待ち | CPU コピー | 分割の合計 | ブロッキング待ちの wall |
|---|---|---|---|---|
| 1920×1080 | 0.64 / 0.66 ms | 0.61 / 0.72 ms | 1.25 / 1.38 ms | 2.36 / 2.44 ms |
| 3840×2160 | 2.77 / 2.28 ms | 3.14 / 2.87 ms | 5.91 / 5.15 ms | 7.58 / 6.23 ms |

（2 回の実行値をスラッシュ区切りで併記）

分割の合計が wall より 1.1 ms 前後小さいのは、分割側が
`is_complete()` のスピンで待つのに対し `to_frame_buffer` は
`device.poll(Wait { submission_index })` でスレッドをブロックするため。
差分はブロック→再開のレイテンシで、**GPU コピーそのものよりも大きい**
（1080p では 0.64 ms のコピー待ちに対し 1.1 ms）。ここは非同期化とは別に
拾える余地があるが、スピンは 1 コアを焼くので「待っている間に別の仕事が
ある」形（＝まさに非同期化）でなければ意味がない。

#### 判断: `GPUCOMP-10`（非同期リードバック）は現時点では不要

根拠:

1. **1080p で 2.4 ms/フレーム**。60 fps 予算 16.7 ms の 14%（変更前は
   6.1 ms = 37%）。しかもアプリの対話評価は `VIEWER_MAX_DIM = 1024` に
   制限されているので、実際の表示経路はこれより小さい解像度で動いている。
2. **非同期化で隠せるのは wall から CPU コピーを引いた分**、つまり
   1080p で 1.7 ms / 4K で 3.4–4.4 ms。残る CPU コピー（1080p 0.6–0.7 ms、
   4K 2.9–3.1 ms）はどう並べ替えても消えない。1.7 ms の回収に対して、
...
`map_async` もコールバックも外に出さない形で完了状態を表しているので、
`GPUCOMP-10` に踏み込む場合の追加は「いつ取りに行くか」のスケジューリングだけで、
公開 API の作り直しは要らない。
