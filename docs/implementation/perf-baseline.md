...
新経路の 8.1 ms は **ingest 7.2〜7.3 ms + scale 0.33 ms + デコード約 0.5 ms** で、
**ingest が全体の約 89%**。`HIGH-32` 節の u8 PQ の 7.211 ms と一致する
（あの節の初出値が 3 倍過大だったことは同節の訂正に書いた。この一致は補正後の
値でのみ成立する）。

HIGH-17 は色変換そのものを変更していないので **ingest の床はそのまま残り**、
今回削れたのは scaler 生成と出力バッファ確保の 0.551 ms — 全体の 6.4% にとどまる。
