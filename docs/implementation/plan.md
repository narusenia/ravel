...
|----|----------|------|--------|------|-----------|
| MS1 | Foundation | コアエンジン基盤、GPUIシェル、プロジェクトファイル | 10 | 8/10 | In Progress |
...
| TASK-006 | GPUIアプリケーションシェル | L | 005 | 🟡 部分完了（ブート+メニュー+DockArea） |
| TASK-006a | GPUIパネルトグル+プリセット切替 | M | 006 | ✅ Done |
| TASK-006b | GPUIデタッチ/復帰+Windows CI | M | 006a | 🔲 Not Started |
...
> **TASK-006a完了（2026-06-24）**: パネルトグル＋プリセット切替のGPUI実結線完了。View/Workspaceメニュー連動、DockArea再構築、Outliner/Dopesheetパネル追加。テスト222pass。
> メニューチェックマークはモデル層で正常動作、GPUI描画側制約で未反映（将来カスタムメニューで対応）。
> **残作業（TASK-006b）**: 実デタッチウィンドウ化・復帰、Windows CIビルド。
