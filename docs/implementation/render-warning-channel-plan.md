...
- `Warning` に 2 つ足す:
  - **オフラインの素材**（`HIGH-34`）。粒度は `AudioSourceSkipped` に揃える
    — レイヤー名 + 素材の表示名 + 理由。**素材ごとに 1 行**
  - **効いていない識別子ポート**（`HIGH-35`）。ノードとパラメータキーと
    「保存値を使った」旨
...
- オフラインの素材を含む文書のレンダーで、レポートと `--json` の**両方**に
  素材ごとに 1 行出る
- 識別子ポートを持つ文書のレンダーで、効いていないポートが 1 行出る
- **終了コードは変わらない**（警告は失敗ではない）
- 素材が全部解決する文書で 1 行も増えない（既存の出力が変わらないこと）
- 同じ素材を 3 レイヤーが参照しているとき、行が**素材ごとに 1 行**に畳まれる

### 単位 3: ロケール / 文書 / issue の決着

**完了条件**

- ロケールが ja / en 揃っている（レポートに出る文字列を足した場合）
