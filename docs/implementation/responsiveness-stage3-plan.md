...
> MediaBin 側を片付けた）。**未解決のまま残るのは `MED-UI-02` の 1 件**
> （パネル非表示のときスキップが未達。`ravel-dock` の可視性配線が要る）。
...
> 正しい（削ると値が止まる）。
>
> **未達なのは「パネルが非表示のとき `refresh_values` を走らせない」。**
> タブの可視性がパネルへ届く仕組みが無い — `ravel-dock` はタブの
> `active: usize` を持つが、それを `PaneContent` 側へ伝えない。配線は
> ドッキング層とパネル層をまたぐので**設計ゲート規模**であり、この単位の中で
> やる作業ではない。この 1 点があるためフェーズ C3 は `完了` にできず、
> `MED-UI-02` も未解決のまま残してある。
