...
- [ ] `ChannelSource` enumが全バリアントで定義されている
- [ ] `AnimationChannel`がフレーム番号から値を評価できる
- [ ] ベジエ補間が正確に動作する（精度テスト: 許容誤差1e-4以内）
- [ ] リニア補間が正確に動作する（ユニットテスト）
- [ ] ステップ補間が正確に動作する（ユニットテスト）
- [ ] 範囲外アクセスで最初/最後の値が返される（ユニットテスト）
- [ ] キーフレームの挿入/削除/修正が動作する
- [ ] 空カーブの評価がデフォルト値を返す（ユニットテスト）
- [ ] Blendソースで2ソースのブレンドが動作する（ユニットテスト）
- [ ] placeholderソース（Expression, AudioReactive）がパニックせずデフォルト値を返す
