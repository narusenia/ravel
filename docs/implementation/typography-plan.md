...
> **Status**: `TYPE-1` / `TYPE-2` / `TYPE-4` / `TYPE-5` / `TYPE-6` 実装済み
> （#506 / #508 / #523 / #511 / #518）— 2026-09-07。
> **残るのは `TYPE-3` と `TYPE-7` で、どちらも `KIT-1` 待ち。**
...
実装済み。`text.on_path` ノード（入力 2 出力 1）+
`ravel_core::geometry::PathArcTable`。

- **弧長は `advance` の累積で決める。** `P` は書字軸成分と交差軸成分の
  組で、縦組みではその 2 つが入れ替わる（`P.x` が列の座標になる）ので、
  成分を固定で選ぶと縦組みが 1 点に積み上がって黙って壊れる。
  `advance` は「書字軸方向の送り」と定義されていて（`names::ADVANCE`）
  書字モードを問わないため、こちらを使う。代償は `text.layout` 側の
  `align` / `anchor` と行の分離が効かなくなること — 複数行は 1 本の
  連続した流れとしてパスに乗る（パス上に交差軸の置き場が無い）
- **書き換えるのは `P` と `rot` だけ。** `instance_sources` /
  `source_index` と per-character アトリビュートは素通しなので、
  単位 6 のフィールド変調の入口はノードの前後で変わらない
- **セグメント表は文字ループの外で 1 回作る。** `path_sample` を文字ごとに
  呼ぶと文字数 × 頂点数になるので、弧長の走査を `PathArcTable` として
  公開し、`path_sample` はその 1 回分のラッパにした（挙動は不変）。
  表の中の探索も線形走査から二分探索に変えている（累積長は単調）
- **パス長 < テキスト長はクランプ。** `PathArcTable::sample` が弧長を
  `0..=length()` に丸めるので、あふれた文字はパス末端の点に積まれる
  （外挿も脱落もしない）
