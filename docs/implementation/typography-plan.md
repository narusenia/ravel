...
- 縦中横（半角数字の横倒し）は**非対象**。
- 禁則処理: 行頭禁則（`。、）」` 等）と行末禁則（`（「` 等）を
  **追い出しだけ**で解く（**ぶら下げは非対象**）。JIS X 4051 の完全実装は
  目指さず、一般的な禁則文字テーブルによる追い出しのみ。
...
実装済み。`text.layout` に `writing_mode`（`horizontal` / `vertical`）を
1 つ足しただけで、専用のノードもパラメータ群も増やしていない。

- **縦書きは横書きの座標を入れ替えたもの**。`WritingMode::Vertical` は
  `vertical-rl` 固定で、配置の式は横書きと同一のまま「行方向の pen」と
  「行を跨ぐ軸」を X / Y のどちらに書くかだけが変わる。`vertical-lr` と
  sideways は入れていない（対象スクリプトに用途が無く、配置の第 3 経路に
  なる）
- **`align` と `anchor` は意味を保ったまま軸が入れ替わる**。`align` は
  「文字が進む軸の上でどこに行を置くか」なので縦では列内の上下位置に、
  `anchor` は「行が積まれる軸の上でどこにブロックを置くか」なので縦では
  列を横切る左右位置になる。列は右から左へ進むので、`anchor` の
  オフセットと `leading` の符号を反転させるだけで済む
  （`Top` = ブロックの右端が原点、`Bottom` = 左端が原点）。
  `wrap_width` は列の長さになる
- **`vert` / `vrt2` は両方渡す**。rustybuzz は縦方向バッファに `vert` を
  自前で有効化するが `vrt2` は意図的に有効化しない（harfbuzz は回転を
  クライアント任せにしている）ので、`vrt2` はこちらから要求する。片方しか
  持たないフェイスがあるので両方渡す
- **`vhea` / `vmtx` を持たないフェイスは 1 em 幅の列になる**（中心線が
  真ん中）。多くの欧文フェイス — 同梱の Geist も — が該当する。字送りの
  フォールバックは rustybuzz 側が `ascender - descender` で持っているので、
  こちらで用意するのは列の幅だけでよい
- **禁則は UAX #14 の穴を埋めるもの**。`unicode-linebreak` は
  `× CL` / `OP ×` / `× NS` を既に守るので、行分割の候補がある限り
  行頭・行末禁則は自動的に満たされる。埋めるのは
  **候補が無いときの緊急切り出し**（`wrap_paragraph` の
  `last_break.unwrap_or(index)`）で、そこだけが禁則文字の位置を見ずに
  切っていた。`kinsoku_cut` が追い出しで 1 つずつ戻し、行が空になる手前
  （`start + 1`）で打ち切る。ぶら下げ・追い込み・縦中横は非対象
- **禁則文字テーブルに ASCII は入れない**。欧文段落の折り返しが
  禁則の導入前と 1 文字も変わらないことを、これで構造的に保証している
