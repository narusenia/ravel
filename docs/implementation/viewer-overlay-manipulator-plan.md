...
### 単位 3: Geometry オーバーレイと `shape_node_bounds` の廃止 — 完了
...
`(path, graph, node, ctx)` を自分で持ち、ワーカーは**同じ `Evaluator`** で
`evaluate_at` を続けて呼ぶ。`EvalUpdate` は対になる
`scoped: Vec<ScopedResult>` で返し、**`results` の位置規約は不変** —
`results[0]` はコンプ出力のままなので `ViewerUpdate::from_eval` は触っていない。

`ctx` を持たせたのは、`comp.network` がレイヤーネットワークへ入るときの
コンテキストがレイヤーローカル時間だからである（REQ-LAYER-006）。要求自身の
コンテキストで引くと**フレームがずれるうえキャッシュも外す** — シェル評価が
たった今埋めたエントリと別のキーになる。`[in, out)` の外のレイヤーは
シェルが評価しないので、ターゲット自体を出さない。

**結果の索引**は `(Vec<PathSegment>, NodeId)`。`NodeId` 単独では同一性に
ならず、2 レイヤーを同時に選ぶと片方の結果がもう片方として描かれる。

**描画側**: `crates/ravel-app/src/panels/viewer/geometry.rs` が評価済み
`Geometry` から bbox（点ドメイン ∪ インスタンスドメイン）・点・パスを読む。
`GeometryOverlay`（旧 `SelectionBboxOverlay`）が対象を宣言し、bbox / 点 /
パスをそれぞれのトグルで描く。**属性値のトグルは単位 8 の担当**なので入れて
いない（矢印・index ラベル・group 色分けと同じ層）。

`shape_node_bounds` は削除した。クリック判定（`hit_test_shape_nodes`）と
レイヤードラッグ（`layer_geometry_nodes`）も同じ評価済み bounds を読む。
レイヤー bbox は**終端のジオメトリノード**だけを union する
（`terminal_geometry_nodes`）— `shape.rect → geometry.transform` の両方を
足すと、レイヤーが決して表示しない矩形を描くことになる。

`is_active` は選択で決まり、結果では決まらない。結果で決めると
「非アクティブなオーバーレイには対象を訊かない」ため要求が永遠に出ない。
**結果未着なら `paint` が何も描かず、クリックも当たらない。**

**単位 7 の制約は解消していない**: `layer_comp_rect` はメディア / エフェクト
のみのレイヤーでは今も `None` を返す。ジオメトリを置かないレイヤーには測る
ものが無く、レイヤーネットワークの出力（`net.out`）はコンプ解像度の
フレームなので、そこから出る矩形はコンプ全体で bbox の役に立たない。
ソース寸法を持つのはメディアアセット側であって評価結果ではないため、
別単位の仕事になる。
...
**グリッドは表示サイズから、上限は `MAX_FIELD_SAMPLES`（4096）**。
両軸を同じ係数で縮めるので横長のビューアが正方形に潰れない。上限が要るのは
遅いからではなく止まるからで、`GradientField` は 1 点につきソースを 4 回
サンプルし、連鎖すると掛け算になる。サンプルは**グリッド全体で 1 回の
`Field::sample` 呼び出し**（API が列志向なのはこのため）。

**座標はネットワークのもの**。グリッドはコンプ空間に敷き、各点を殻の
**逆行列**でレイヤーローカルへ戻してからサンプルする。殻が退化していて
逆行列が無いレイヤーは描かない。

**等値線はセル解像度**で、marching squares ではない。値を 8 段に量子化し、
右または下の隣と段が違うセルだけを塗る。線は 1 セル幅で階段状になる —
フィールドの形を一目で掴むという用途には足り、滑らかな等値線を出すには
辺の内挿とセグメント列が要る。

**正規化はグリッド自身の範囲**（オートレンジ）。ノイズは `-1..=1`、falloff は
`0..=1`、式フィールドは任意なので、固定レンジではほとんどが平坦に見える。

**描くのは選択が 1 ノードのときだけ。** ヒートマップを 2 枚重ねてもどちらの
ことも分からない。矢印はベクトル値のフィールドにのみ出る（スカラーに向きは
無い）。**結果未着なら描かない。**
...
- **既存の「bbox 内側ドラッグで移動」は機構へ載せ替えていない。** 代わりに
  bbox 中央へ移動グリップを置いた。内側の面全体を掴むには `OverlayHandle` に
  矩形のヒット形状が要るうえ、そのハンドルが**レイヤー内のノードのクリック
  選択を潰す**（`overlay_handle_mouse_down` が `select_mouse_down` より先に
  return する）。既存の複数レイヤー移動の挙動も変わる。載せ替えるなら
  ヒット形状とイベント順序の設計を伴うので、独立した単位にする
- **殻マニピュレータは自前の印を出す。** ノード選択の bbox が描く 8 ハンドルは
  飾りのまま残る。位置は同じ `selection_handle_centers` を使うので見た目は
  一致するが、ノードの箱から殻を拡大縮小するのは意味がずれる
  （ノードのサイズを書くのは単位 5 の `ParamRole`）
- **回転カーソルは `DragLink` で代用。** GPUI-CE に回転カーソルが無く、
  ビットマップカーソルは gpui-ce へのパッチが前提になる
- **HUD の置き場所だけがキャンバス左上に固定**（ポインタ追従ではない）。
  `LabelPlacement` にコンプ空間アンカーが無く、その追加は単位 8 の担当。
  中身は計画どおりデルタで、スケールはそのドラッグが掛けた倍率、回転は掃いた
  角度、位置とアンカーは座標を出す
- **Shift は scale にのみ効く。** 回転の角度スナップと移動の軸拘束は未実装
- **ジオメトリを置かないレイヤー**（メディア / エフェクトのみ）は対象外。
  `layer_comp_rect` が `None` を返すため。**単位 3 でも解消しなかった** —
  評価済みジオメトリが無いレイヤーには測るものが無い（同単位の「実装」節）

### 単位 9: モーションパス（軌跡とキー位置のドラッグ） — 完了
...
`crates/ravel-app/src/panels/viewer/motion_path.rs`。`MotionPath::resolve` が
折れ線とキー点を一度に出し、`paint` / `handles` / `drag` はそれを読む
（描いた点と掴める点が同じ値であることを構造で保証する）。

- **評価要求は出さない。** `eval_targets` はトレイト既定のまま。テストは
  「active なコンテキストで空」と「position をキーにしてもレジストリ全体の
  ターゲットが増えない」の 2 本で固定する。fixture のレイヤーには
  `GEOMETRY` 出力を持つノードを置いた — ネットワークが空だと
  「ターゲットを返す」変異が検出できない
- **区間は `[in_frame, out_frame)` から作る**。「今表示中か」を問わないので
  `local_frame` の clamp を踏む場所が無い（軌跡は playhead が乗っていない
  フレームの話なので、そもそも区間判定をしない）
- **サンプル上限 `MAX_MOTION_SAMPLES`（256）**。間引いたうえで**末尾フレームを
  必ず含める**ので、軌跡がレイヤーの終わりより手前で切れない。キー点も同じ
  上限で間引く
- **キー点のドラッグは `position` の両成分を、掴んだキー自身のフレームへ**
  書く（`local_frame: Some(key_frame)`。playhead のフレームではない）。1
  ジェスチャ 1 undo は既存の `handle_drag_ended` 経路がそのまま担う
- **親を持つレイヤーは親の行列越し**に描き、ドラッグはその逆行列で戻す。
  行列は単位 7 と同じく**画面のフレームでの姿勢**を使う
- 優先度は `MOTION_PATH`（36）= 殻マニピュレータの上、`ParamManipulator` の
  下。playhead がキーに乗っているとアンカーマーカーとキー点が同じ位置に来る
  ため、より具体的なキー点が押下を取る
- 区間外のキーは描かない（軌跡が届かない位置に印だけ出ることになる）

### 単位 8: ジオメトリ属性の空間可視化 — 完了
...
`GeometryOverlay` に載せた（新しいオーバーレイは作っていない）。要素の
**単位は `GeometryMark`**（domain / 要素 index / 位置）1 つで、点マーカー・
矢印・index ラベル・group 色分けはすべてこの列から派生する。属性列を読む行が
位置を取った行と同じであることが構造で保証される。

- **矢印は属性そのものの長さ**。上限 `reach`（コンプ短辺の 15%）は
  `min(1, reach / longest)` として**縮める方向にだけ**効くので、小さい
  velocity は 1:1 で読める。非有限値はスキップ（`longest` に混ぜない）
- **index ラベルは `LabelPlacement::Comp`**（この単位で追加したコンプ空間
  アンカー。単位 7 が HUD で見送った置き場所の一般化）。上限
  `MAX_DRAWN_LABELS`（64）で**間引き**、truncate しない
- **group は Bool 属性**（`procedural-geometry.md` 要素スコープ）。色は
  **group 名のハッシュ**から取るので、並ぶ group が増えても domain が違っても
  同じ group は同じ色
- **矢印の対象属性はピッカーで選ぶ**。候補は評価済みジオメトリが実際に持つ
  Vec2 / Vec3 / Vec4 列（`P` は除く）で、予約名の固定リストではない。
  `particle-plan.md` の velocity 表示はこれに乗るだけで専用経路を持たない
- トグルは `paint` / `labels` だけが読む。`is_active` / `eval_targets` は
  従来どおり選択だけで決まる（要求組み立て時のコンテキストにパネルの
  トグルは無い。単位 4 と同じ理由）
