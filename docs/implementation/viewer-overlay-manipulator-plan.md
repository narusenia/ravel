...
単位 2 が載せた相乗りは、**viewer の要求が評価するグラフに実在するノード
だけ**を対象にできる。viewer の `EvalRequest` は `graph` にシェルの
コンパイル済みグラフ、`path` に空（root スコープ）を入れるので、
**レイヤーネットワーク内部のノードは原理的に相乗りできない**。
`EvalRequest` は 1 リクエスト = 1 グラフ + 1 パスだからである。
...
入れた機構はコンプスコープのターゲットに対しては正しく動き、テストで
固定してある（組み込みオーバーレイはまだ `eval_target()` を宣言しないので
production では現状ドーマント）。
