...
この issue が挙げた 4 つの症状のうち **3 つは解決済み**で、残るのは往復だけ。
...
`CM-7` 後の 1920×1080、交互測定（`perf-baseline.md`）:

| 区間 | コスト |
|---|---|
| GPU 常駐フレームが画面に届くまで（`CM-7` 後） | **2.14 ms** |
| うちリードバック（`GPUBK-6` 後の実測、1080p） | **約 2.2〜2.4 ms** |

**つまり残りはほぼ全部が往復。** 変換を GPU へ移した今、この 2 ms 強を消す
手段は往復を無くすことしかない。60 fps 予算 16.7 ms の 13%。
...
- リードバックの 2.2〜2.4 ms は `GPUBK-6` 後・`CM-7` 前の数字で、**表示変換が
  GPU へ移った後の内訳は測り直していない**
- 往復を消しても、GPUI 側のアトラス churn（`HIGH-09` の 4 つ目の症状）が
  残るなら得は小さい
...
- macOS で `MTLDevice` / `MTLCommandQueue` が取れ、Ravel の
  `interop::context_from_wgpu` に渡せる
...
- `HIGH-09` を `issues/closed/` へ（**4 症状すべてが解決してから**）
...
- **アトラス churn の解消**（`HIGH-09` の 4 つ目）。カスタム要素で描けば
