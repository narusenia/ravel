...
**無効化はどのコンプかとどのフレームかを別々に決める。** どのコンプかは
文書差分（`Document::compositions` は `Arc<Composition>` なので、触っていない
コンプはポインタ一致で無料）。どのフレームかは、編集が
`InvalidationHint::Params` としてノードを名指ししていて、その全ノードを
所有レイヤーへ解決できたときだけ、そのレイヤーの
`[start_frame, start_frame + 尺)` に絞る。名指しの無い編集は従来どおり
コンプ全破棄で、絞り込みは安全側の既定の上に載る最適化にとどまる。
precomp で他コンプに置かれたコンプの編集は、**それを含むレイヤーの範囲**
として親へ伝播する（親コンプ自身の `Arc` は動かないため、伝播しなければ
親が編集前の絵を返す）。

**先読み（投機充填）** は、評価ワーカーが一定時間なにも要求されていない
ときにプレイヘッド前方のフレームを埋める。投機は対話要求とは別のキューに
入り、対話要求が来たら残りは破棄され、**すでに評価中のフレームも次の
`process()` の手前で打ち切る**（判定はチャネルが空かどうかで、時計は見ない）。
世代番号を進めず、結果の通知（`on_update`）も出さない — キャッシュに書く
だけなので、ビューアの表示が勝手に差し替わることはない。予算の退避順では、
**先読みが作ったフレームキャッシュのエントリと評価器のノード結果**が
通常エントリより先に落ちる。共有デコードキャッシュはアセット単位で対話評価と
共有されるので、投機ランクの対象ではない（同じデコード結果を対話側も使う）。

単一の予算は `ravel_core::cache_budget::CacheBudget` が持つ。層ごとに
バイト上限を持ち、`reserve(kind, bytes)` が上限超過時に**退避対象のリスト**を
返す（実際に落とすのは呼び出し側）。退避順は
`投機で入ったエントリ → 通常エントリ（最終アクセス順）` で（投機ランクで
入るのは先読みが作ったフレームとノード結果）、sim の予約枠は
通常エントリの圧力では削られない（sim の再計算はフレーム数に比例し、
他の層と非対称なため）。VRAM は「キャッシュ保持分 + テクスチャプールの
アイドル枠」の総額で、**アイドル枠は残余**として動的に決まる — プールは
自分の上限を持たない。予算を作れる場所は 1 箇所（`ravel-app` の
`ProjectState::new`）で、`Default` は公開しない。

> **実装状況**: 単一予算（`CacheBudget`）、ノード単位の評価キャッシュの
...
> 無効化のレイヤー時間範囲への絞り込みと precomp 親への伝播（`CACHE-7`）、
> アイドル時の先読み（`CACHE-9`）も実装済み。先読みは既定で無効で、
> `EvalServiceConfig` で入れる — 実際に入れているのは `ravel-app` の
> Viewer 用サービスだけで、レンダーとベンチマークは要求したフレームだけを
> 評価する。
...
- **(B) macOS のレンダラを wgpu へ寄せる、または Metal レベルで interop する。**
  ...
  `ZC-3` / `ZC-4` が描画と寿命を仕上げた。macOS のビューアは完了**
