...
- **Int はアニメーション可能**（`IntChannel`、journal v9 / `.ravprj` v10）:
  ...
  **f32 なので 2^24 を超える整数は厳密でない**（`16_777_217` は
  `16_777_216` として読み戻る）。`AnimationChannel` を総称化しない
  （参照 382 箇所）代償で、組み込みパラメータはどれもこの天井に届かない
  （繰り返し回数・分割数・列挙の選択）。`i32` の全域をアニメートさせたい
  パラメータが出たら、そのときは整数チャンネルが必要になる。
  Timeline には f32 の行と同じ 1 レーンの行が出て、カーブエディタでは
  **丸めた値の階段**として描かれる（制御点は f32 のまま掴める）。
- **String もアニメーション可能**（`StringSteps`、journal v10 / `.ravprj` v11）:
  ...
  **カーブエディタには出ない**。定数の `String` はそのまま残り、キーフレームを
  打った瞬間に再型付けされる。Timeline には**キーの追加・移動・削除だけ**が
  できる 1 レーンの行が出て、**補間の切り替えは出ない**（中間値が無いので
  選ぶものが無い）。最後のキーを消すと `ParameterValue::String` へ戻り、
  戻す値はカーブの**既定値**（キーを打つ前の定数）である。
  **Bool は定数のみ**（`StepCurve<T>` が機構としては
...
- **`Save As` でプロジェクトルートが変わったとき**、新しいルートを基準に
  ...
  新しい `${PROJECT_ROOT}` で解決し直す — 変数パスを設定できる意味はそこに
  ある。オフラインのエントリは書き換える `resolved` を持たないので保存された
  パスを保ち、新しいルートがそれに答えられればオンラインに戻る。
  **これは編集ではない**: `revision` を進めず undo にも積まない
  （ディスク上の書庫が既にその形を持っているので「保存された内容と一致」は
  保たれる）。undo 履歴に残っている版も同じ写像を通すので、undo が旧ルート
  基準の読みを復活させることはない。
- したがって「保存 → プロジェクトディレクトリごと移動 → 再オープン」で
  参照は解決したままになり、「保存 → ロード → 保存」はバイト一致する。
- **「オフライン」は参照が解決しないことだけを意味する**（`resolved == None`。
  `media` ノードや `AudioSource` が持つ `AssetId` をアセット表が持っていない
  場合も、参照としては同じくオフライン）。`AssetPath::resolve` はディスクを
  触らないので、**解決するパスのファイルを消しただけではオフラインにならない**
  — 次にそのパスを解決し直すもの（ロード / `Save As` / Relink）が走るまでは
  オンラインのままで、評価は `media` ノードのデコード失敗として透明フレームに
  なる。判定を広げるにはバックグラウンドの `stat` とその結果の置き場が必要で、
  描画経路と評価経路に I/O を入れないという制約がそれを別の設計にする。
  UI ではこの判定が MediaBin の行バッジ、Properties のファイルセクション、
  Outliner と Timeline のレイヤー行の印（`media_bin::layer_is_offline`）に
  出る。レイヤーの印は殻の音声ソースと、入れ子のサブネットを含む
  ネットワーク全体の `media` ノードを見る。
