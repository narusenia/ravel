...
`rasterize` は画像インスタンスを受け入れる（Mesh の拒否は従来どおり）。
**ただし今描くのは CPU 経路だけで、GPU 経路は画像インスタンスを警告付きで
飛ばす**（`image-instancing-plan.md` の `IMG-5` が入るまで）。GPU が使える
環境では画像のコピーは絵に出ない。

CPU 経路は texel を読むので、GPU 常駐フレームで来た画像はノード入口で
**ソースごとに 1 回だけ**読み戻す（コピー数には比例しない）。`IMG-5` が入れる
GPU 経路は読み戻さない設計で、この非対称は意図的な制約である。
