...
| `ravel-core/src/registry/builtin.rs` | シェイプ系の出力型変更 + 新ノード登録 | 🔶 rasterize / shape.* / scatter.* テンプレート追加済み、field.* / attribute.* は未（TASK-039） |
| `ravel-nodes` | シェイプ processor のジオメトリ化、Rasterize、フィールド群 | 🔶 shape.*（TASK-043）・scatter.*・Rasterize（CPU）・field 処理系は実装済み、attribute.* は未（TASK-039） |
| `ravel-core/src/composition/compile.rs` | Shape/Text Layer ソースの展開先変更 | 🔶 Shape は `ShapeGeometry → Rasterize` 展開済み、Text は未 |
| `ravel-app`（Node Editor / Properties） | 新型のポート色/接続判定、属性検査 UI | 🔶 GEOMETRY ポート色・Viewer 表示は実装済み、FIELD 色（TASK-039）・属性検査 UI（TASK-047）は未 |
