...
| 箇所 | 改修 |
|------|------|
| `ravel-core/src/types.rs` | `GeometricData` 実装型の追加（現在実装型ゼロ） |
| `ravel-core/src/geometry/`（新設） | Geometry / AttributeSet / Field |
| `ravel-core/src/eval.rs` | sim キャッシュ、`StatefulProcessor` 統合 |
| `ravel-core/src/registry/builtin.rs` | シェイプ系の出力型変更 + 新ノード登録 |
| `ravel-nodes` | シェイプ processor のジオメトリ化、Rasterize、フィールド群 |
