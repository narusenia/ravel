...
- **オフラインのメディアを参照するレイヤー行**には警告アイコンを出す（`MEDIA-7`）。
  レイヤーのネットワーク（入れ子のサブネットを含む）の `media` ノードか、殻の
  音声ソースが、解決しないアセット（アセット表が持っていない `AssetId` も同じ）を
  指しているとき。**印はレイヤー行だけ** — 再リンクの対象はレイヤーが参照する
  素材なので、コンプ行にもノード行にも出さない。判定は Timeline のレイヤー行と
  共有の 1 関数（`media_bin::layer_is_offline`）で、行の構築時に済ませるので
  描画はアセット表もグラフも歩かない。**オフライン = 参照が解決しない**であって
  ファイルの存在は見ない（[`../data-model.md`](../data-model.md)）
...
**タブが手前にない間は行の再構築を走らせない。** コンプ切替もドキュメント編集も、
裏にいる間は借りとして残り、表に戻ったときに 1 回で追いつく
（[`../ui-spec.md`](../ui-spec.md) の設計原則）。
