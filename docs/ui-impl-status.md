...
| レイヤー殻のマニピュレータ | ✅ | **レイヤーを 1 枚だけ選んでいるとき**、その殻 bbox に 8 個のスケールハンドル（角は 2 軸、辺の中点は 1 軸）、角の外周リングに回転、中央に移動、アンカーマーカーを出す（`ShellManipulator`、OVL-7）。**Shift で縦横比固定、Alt でアンカー基準**（既定の固定点は対角のハンドル。`position` が補正を吸収する）。アンカーのドラッグは `anchor_point` と `position` を同時に書くので**見た目が動かない**。キーフレーム付きチャネルは平坦化せず、そのレイヤーのローカルフレームにキーを打つ（REQ-LAYER-006）。親を持つレイヤーは親チェーンの行列込みで表示・編集する。**1 ドラッグ = 1 undo、Esc で revert**。ドラッグ中はキャンバス左上に数値 HUD（倍率 / 角度 / 座標）。shape ノードの bounds が測れないレイヤー（メディア / エフェクトのみ）は対象外（OVL-3 待ち） |
