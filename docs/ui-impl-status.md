...
| チャンネル単独表示 | ✅ | ツールバーのセレクトとコマンド（`viewer.channel_*`、`Alt+Shift+C` / `R` / `G` / `B` / `A`）で RGB / R / G / B / アルファ / アルファマットを選ぶ（`INSP-2`、REQ-UI-004）。**評価結果は変えない** — 表示変換 1 パスにモードを u32 で渡すだけで、ホストの `Arc<AtomicU32>` を評価ワーカーの `DisplayTransform` が毎フレーム読む（`EvalContext` には入れないので、ノードのキャッシュ同一性は汚れない）。切り替え時は出力段フレームキャッシュ（`CACHE-5`）の当該コンプ分を捨てて `InvalidationHint::None` で 1 回要求する — このキャッシュは finalize 済みの表示バイト列を持つのでヒットすると前のモードの絵が返る。つまり**再評価ではなく変換のやり直し**（`ravel-core` の `invalidating_the_finished_frames_refinalizes_without_reprocessing` が `process()` は 1 回・`finalize` は 2 回であることを固定）。単独チャンネルはグレースケールかつ不透明。アルファは**マット自身**、アルファマットは**色にマットを掛けた結果**（リニア光で乗算するので 0.5 の色に 0.5 のアルファは 137、符号化後に掛けた 94 ではない）。CPU 定義（`to_display_rgba8_channel`）との一致基準は他と同じ **8bit で ±1 コード**で、6 モード全部を GPU テストが検査する。モードはセッション内だけの状態で `.ravprj` にも `ui_state.json` にも載らない（同じプロジェクトを開き直したときに前回の検査モードが復元されないように）。アルファマットはコマンドを持たずツールバー専用。**実機未確認**（自動テストのみ） |
