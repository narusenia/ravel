...
| ノードパラメータのマニピュレータ | ✅ | `ParamRole` は `Position` / `Size` の 2 種だけ。選択ノードの評価済みジオメトリにハンドルを出し、アニメーション付きでも現在のローカルフレームへ書く。接続で駆動されるパラメータにはハンドルを出さない。ホバー中はその印の位置に**パラメータ名（Properties の行と同じ解決）とパラメータ自身の値**を出す（`Size` は描かれる点ではなくオフセットの値）。`Direction` / `Angle` は未導入 |
