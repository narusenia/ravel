...
| ノードパラメータのマニピュレータ | ✅ | `ParamRole` は `Position` / `Size` の 2 種だけ。選択ノードの評価済みジオメトリにハンドルを出し、アニメーション付きでも現在のローカルフレームへ書く。接続で駆動されるパラメータにはハンドルを出さない。ホバー中とドラッグ中はその印の位置に**パラメータ名（Properties の行と同じ解決）とパラメータ自身の値**を出し（`Size` は描かれる点ではなくオフセットの値）、ドラッグ中は左上に `Δ` と押下からの移動量も出す。押下時の値は index ではなく**キーで突き合わせる**（ドラッグが欠けているパラメータを生やすと宣言順が動くため）。`Direction` / `Angle` は未導入 |
