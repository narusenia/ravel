...
| 再生とキャッシュの状態表示 | ✅ | 画面右上に 1 行（`·` 区切り）で 「今見えている絵は真実ではない」だけを出す（`INSP-4`、REQ-UI-004）。要素は 3 つ: **ドロップフレーム**（`Transport::dropped_frames`。再生開始でリセットされる累積値なので**再生中だけ**。停止したら消える — 終わった区間のカウントはもう確かめられない）、**プレビュー解像度**（実効係数が `Full` より低いときだけ。画面の性質なので停止中も出す。係数はコンプ解像度と評価解像度の対から `ViewerResolution::apply` を逆に当てて復元する — 幅で割ると `div_ceil` の分だけ奇数幅で 1 になる）、**キャッシュ済みの割合**（`CACHE-6` のキャッシュ帯 ÷ コンプ長。**再生中だけ**。停止中は Timeline の帯がフレーム単位で同じことを言うので二重表示にしない。帯が空＝評価前は割合を出さず他の要素だけ出る）。3 つとも無ければラベルを 1 枚も置かない。**書き手は 1 経路**（`PlaybackController::publish_position` が `panels::PlaybackStatus` Global を値が変わったときだけ書く）で、**観測者は置かない** — 再生中はプレイヘッドの再描画が毎フレーム走るので、購読すると `HIGH-21` と同型の二重再描画になる。要素の判断と 1 行の組み立ては `ravel_ui::panels::viewer::playback_status_text`（純粋関数）。**実機未確認**（自動テストのみ） |
