...
| パラメータ表示 | ✅ | key: value 形式、セパレータ線付き。View メニューの「パラメータ値を表示」（`CommandId::ViewToggleNodeParamValues`、既定のキーバインドは無い）で消せる（PGRP-5）。**単位はキャンバス全体**で、消すと行とセパレータが高さごと落ちてノードが縮む。`painting::compute_node_size` と `painting::paint_nodes` は同じ旗を受け取り、当たり判定が読む `node_sizes` はその計算の結果なので、絵とヒットボックスが食い違わない。状態は `ui_state.json`（`show_node_param_values`、既定は「描く」で消したときだけ書く）。メニュー項目にチェックは付かない — メニューのチェックはheadless の `MenuBar` が持つ状態（パネル表示・プリセット）だけで、判断材料はキャンバスそのもの |
...
| パラメータグループ | ✅ | ノードのパラメータを**1 階層**のセクションに割る（PGRP-1〜PGRP-4）。宣言するのは**ノード型**（`NodeTemplate::param_groups`、グループ名 → キーの並び）なので `.ravprj` には何も入らない。例外はネットワークインターフェースの In ノードで、カスタムパラメータはインスタンス側に持つ（`Node::param_groups`、キー → グループ名。割り当てはPorts セクションのカスタム行の「グループ」欄。パラメータを持たないポートには欄が出ない）。**両方あればインスタンスが勝つ**。In ノード以外の非空 `Node::param_groups`（手編集の `.ravprj` のみ）は表示側が無視して型宣言に従う。**宣言に現れないパラメータは宣言されたグループより前**の暗黙セクションなので、宣言の無いノードは 1 枚のまま。宣言の破れは吸収する（存在しないキーは落ち、空のグループはセクションを作らず、同じキーは先に挙げたグループ、同名グループは 1 セクションに統合）。見出しは `node.<type_key>.group.<name>`（ja / en 両方をテストが強制）。組み込みで宣言しているのは**パラメータ 6 個以上の 9 テンプレート**（`attribute.set` / `style.stroke` / `field.apply` / `math.remap` / `math.curve` / `scene.camera` / `scatter.grid` / `scatter.circular` / `scatter.scatter`）で、しきい値は `every_built_in_with_six_or_more_parameters_declares_groups` が強制する |
| パラメータグループの折り畳み | ✅ | 畳んだグループは `(type_key, group)` で `ui_state.json` に持つ（PGRP-3、ノードごとではなく**ノード型**なので選び直しても畳み直さない）。**既定は全展開**で 0 件ならエントリを書かない。パラメータグループ**以外**のセクション（Node Info / Ports / 宣言 / 説明）は畳めない。Accordion は行のクリックでも開閉集合を報告するので、`set_param_group_collapsed` が「変わったか」を返し変わったときだけ再描画する。**保存は明示的な保存に乗る**（折り畳みは `ui_state` を dirty にするだけ。自動保存は `SET-9` で未実装）。**上限**: セクション順はポートの並べ替えに追随しない（`Graph::reorder_ports` は `parameters` を並べ替えない）／インスタンスグループ名にロケールキーそのものを打つと見出しが翻訳される（畳み判定は位置で引くので別セクションを畳む事故は起きない） |
...
| Vector フィールド | ✅ | `Channel2`〜`Channel4` パラメータを成分ごとの ScrubInput で表示・編集（ラベルの下に成分を並べ、幅が足りなければ折り返す）。値ドメインのベクタ定数（`constant.vec2` / `.vec3` / `.vec4`）と、組み込みノードのベクタパラメータ（`shape.*` の `center`、`shape.ellipse` の `radius`、`scatter.grid` の `spacing`、`geometry.transform` の `translate` / `rotation` / `scale` / `pivot`、`transform` の `translate`、`field.falloff` の `center` / `direction`、`scatter.scatter` の `area`、`type` が `vec2` / `vec3` の `attribute.set` の `value`）が到達する。各成分は軸の文字（`X` / `Y` / `Z` / `W`）を伴う。**4 成分も到達する**: レジストリが色として宣言していない `Channel4`（`constant.vec4` の `value`、`attribute.set` の `type = "vec4"`）は 4 成分の Vector 行で、色として宣言されたもの（`constant.color` / `style.fill` / `style.stroke` / `rasterize` の `color`、`attribute.set` の `type = "color"`）だけが Color 行になる。**宣言を持てないネットワーク In ノードと subnet ノードの `Channel4` は無条件に色**（カスタムポート型に `Vec4` が無いため）。リンクトグル（均一スケール）は未実装（LOW-APP-31） |
...
| Bool 編集（レイヤー） | ✅ | solo/muted/locked/adjustment を Ravel 自前の `Checkbox`（`ravel-widgets/src/checkbox.rs`、`UIX-9`）で編集。14px の四角、未チェックは枠だけ、チェック済みは `primary` の面に `background` でマークを抜く。disabled でもチェックの塗りは 38% で残す（消すと未チェックと同じ絵になるため）。Tab で到達し Enter / Space がクリックと同じ経路で届く |
...
| Select でパラメータ変更 | ✅ | Enum パラメータ (merge operation、`attribute.set` の `type` 等)。`type` の変更は `value` のアリティも変え、露出済みパラメータポートの型を追随させる（合わなくなったエッジは破棄。値・ポート・エッジで 1 undo）。**候補は文脈依存にもできる**（`ParamOptions::Contextual`）: `layer.ref` の `layer` は同じコンポの他レイヤーを `"{row}. {name}"` で並べ、選ぶとその `LayerId` を書く（1 undo、`CPO-2`）。`port` は参照先レイヤーの出力ポートを並べ、**選んだポートの型がこのノードの出力型になる**（`layer` を変えても追随。運べなくなったエッジは破棄され、値・ポート・エッジで 1 undo。参照が解決できないときは型を変えない、`CPO-3` / `CPO-4`）。候補が空のときは dropdown を出さず理由を読み取り専用行で出し、候補に無い保存値（別プロジェクトから貼った参照、参照先が持っていないポート）は候補の末尾に加えて選択状態を保つ |
...
| キーフレームトグル (◆/◇) | ✅ | アニメート可能フィールド左のダイヤボタンで現在フレームにキー追加/削除（1 undo）。殻 Transform/Opacity/Audio Gain・custom.*（Float / Int）・ノード Float/Channel*/Int/String 対象。定数 Float は Channel 化、定数 Int は `IntChannel` 化、定数 String は `StringSteps` 化（REQ-LAYER-004、discrete-keyframes 計画 DISK-3）。**最後のキーを消すと定数へ戻る** — Float / Int はチャネルが定数ソースに戻り、String は `ParameterValue::String` へ戻る。**識別子パラメータ（`layer.ref` の `layer`、`precomp` の `comp_id`、`media` の `asset_id`）にはトグルを出さない** — 生の ID なのでアニメーションさせると `id_watermarks` が予約すべき ID を決められない（判定は `ravel_core::composition::validate::is_identifier_parameter` の 1 箇所）。Timeline 側でも同じ扱いで、識別子パラメータには行が出ない（`ravel_ui::keyframes::property_rows`）。手編集でそうなっている文書は開けるが、**参照は解決しない**（`ParameterValue::identifier`） |
...
| キーフレームダイヤ | ✅ | 行のキーをレイヤーローカル→Comp 時間へ変換して描画（`comp_frame_for_key`、in_frame 考慮）。選択中は描き分け。列挙は行種別に依らず `keyframes::row_key_frames` の 1 経路（`Keyframes` チャンネルと `StepCurve` の両方） |
| 再生ヘッド | ✅ | 赤色 2px 縦線 |
| コンポ終端の帯 | ✅ | Duration の外をルーラーとレーンで落とす（背景ウォッシュ + tint + 終端 1px 線）。ズームは終端で止めない。グラフモードには敷かない |
| グラフモードのカーブ描画 | ✅ | 選択チャンネルの `KeyframeCurve` を整数フレームでサンプリングした折れ線（ベジエ評価は `KeyframeCurve::sample` の 1 経路）。**`IntChannel` の行は丸めた値の階段**として描く — 段の境界は `round()` が変わるフレームで、水平→垂直の 2 セグメントを打つので隣接フレーム間が斜線にならない。制御点は f32 のままなので掴んで動かすと段が動く。`StringSteps` の行はカーブを持たないのでグラフモードに出ない（`ParameterValue::channels()` が `None`） |
...
| 値ズーム (Cmd/Ctrl+Shift+スクロール) | ✅ | グラフモードのみ。ポインタ下の値をアンカーに表示範囲を固定（`CurveValueRange`、Properties のカーブエディタと同じ型）。Fit でデータ追従へ戻り、追従中の Fit ボタンは無効 |
...
| キーフレーム選択・移動 | ✅ | ダイヤクリックで選択+ドラッグ移動（live apply、mouse-up で 1 undo）。空所クリックで選択解除。**Step 行（`StringSteps`）も追加・移動・削除ができる**が、補間の切り替えは選択に Step キーが 1 つ混ざった時点でツールバーもコンテキストメニューも閉じる — 保持値に中間値が無いので選ぶものが無い |
...
| 項目 | 状態 | 備考 |
|------|------|------|
| FrameBuffer 表示 | ✅ | `ViewerFrame` Global 経由。**macOS は GPU テクスチャを GPUI の surface としてそのまま描き**（`ZC-2`〜`ZC-4`）、Linux / Windows も wgpu surface 経路（`ZC-7` / `ZC-8`）でフレームを CPU に降ろさず描く — いずれもリードバック 0 回。テクスチャは GPUI の描画完了通知でプールへ返る。共有デバイスが取れない場合（複数 GPU 機など）は従来どおり `RenderImage` を `img` 要素で描く CPU 経路にフォールバックする。デバイス喪失はセッションで 1 度だけ通知して再起動を促し、macOS では自前 device の喪失で zero-copy を止めて CPU 経路へ落ちる（GPUI 側 Metal renderer の喪失は検出しない — `GPULOSS-4`）。Linux / FreeBSD / Windows は採用したデバイスの復旧を追いかけて新しいデバイスで評価を作り直す（`GPULOSS-3`、**実機未確認**）。どちらの経路でも表示バイト列は GPU で出来上がっている（`DisplayFrame`） |
...
| プレビュー解像度の選択 | ✅ | ツールバーのセレクトと `View ▸ プレビュー解像度を切り替え`（`Alt+R`）で フル / 1/2 / 1/4 を選ぶ（`VRES-2`、REQ-UI-004）。係数はコンプ解像度に掛かるので 1080p でも 8K でも意味が同じで、`Full` は等倍で結果を確認するための選択。既定は 1/2（1080p で 960x540 ≒ 撤去した 1024px 上限のコスト。理由は `ViewerResolution` の doc）。係数は `ProjectState` が持ち、評価要求はこの実効係数で組む。選んだ係数は `ui_state.json` に載る（`VRES-3`。既定の 1/2 はエントリを書かず、読めない値はその 1 フィールドだけ既定に落ちる。File ▸ New で既定へ戻る）。操作中は自動で 1 段落ちる（`VRES-4`）。落とす合図は 2 つの漏斗だけ — ライブ編集（`apply_document`。全パネルのドラッグとスクラブ値がここを通る）とプレイヘッドのスクラブ（`seek_from_timeline`）。最後の入力から 120 ms で選択係数へ戻して再評価を **1 回**投げる（世代でタイマーを管理するので、ドラッグ中に古いタイマーが戻すことはない）。コミットした単発編集と**再生の tick は落とさない**（1 回の評価で済むものを二重に払わないため／再生中ずっと粗くしないため）。`1/4` を選んでいるときは適応そのものが無効（これ以上落とせない）。落ちている間はツールバーが選択係数と実効係数を並べて出す。**実機確認済み**（2026-08-24、macOS / `cliclick`）: 100% 表示で `1/4` は図形の縁が数ピクセルのにじみ、`Full` は 1 ピクセルの硬い境界。ルーラーのスクラブ中はツールバーが `フル → 1/2` を出し、離すと `フル` に戻る。`Alt+R` でも係数が変わる。**ルーラーの単発クリックと `Home` / `End` も 1 段落ちる**（120 ms で戻る。1 段では体感差が小さく、クリックはドラッグの起点であることが多いので許容した） |
| チャンネル単独表示 | ✅ | ツールバーのセレクトとコマンド（`viewer.channel_*`、`Alt+Shift+P` / `R` / `G` / `B` / `A`）で RGB / R / G / B / アルファ / アルファマットを選ぶ（`INSP-2`、REQ-UI-004）。**評価結果は変えない** — 表示変換 1 パスにモードを u32 で渡すだけで、ホストの `Arc<AtomicU32>` を評価ワーカーの `DisplayTransform` が毎フレーム読む（`EvalContext` には入れないので、ノードのキャッシュ同一性は汚れない）。切り替え時は出力段フレームキャッシュ（`CACHE-5`）を**全コンプ分**捨てて `InvalidationHint::None` で 1 回要求する — このキャッシュは finalize 済みの表示バイト列を持つのでヒットすると前のモードの絵が返り、係数と違って**チャンネルはビューア全体の設定**なので他のコンプの完成フレームも同じだけ古い。つまり**再評価ではなく変換のやり直し**（`ravel-core` の `invalidating_the_finished_frames_refinalizes_without_reprocessing` が `process()` は 1 回・`finalize` は 2 回であることを固定）。単独チャンネルはグレースケールかつ不透明。アルファは**マット自身**、アルファマットは**色にマットを掛けた結果**（リニア光で乗算するので 0.5 の色に 0.5 のアルファは 137、符号化後に掛けた 94 ではない）。CPU 定義（`to_display_rgba8_channel`）との一致基準は他と同じ **8bit で ±1 コード**で、6 モード全部を GPU テストが検査する。モードはセッション内だけの状態で `.ravprj` にも `ui_state.json` にも載らない（同じプロジェクトを開き直したときに前回の検査モードが復元されないように）。アルファマットはコマンドを持たずツールバー専用。**実機未確認**（自動テストのみ） |
| ピクセル値の読み取り | ✅ | ツールバーのボタンとコマンド（`viewer.pixel_readout` / `viewer.pixel_readout_format`、`Alt+Shift+I` / `Alt+Shift+F`）で、ポインタ位置のコンプ座標とその点の RGBA を画面左下に出す（`INSP-3`、REQ-UI-004）。出す値は**評価結果そのもの**（リニア、表示変換前）で、形式は f32（既定）と 8bit 換算。8bit は同じ値を 0-255 の目盛りに置き換えるだけで、画面の表示バイト列ではない（表示変換はユーザーの LUT 込みで評価ワーカー側にあり、UI で近似し直すとグラフのどこにも無い値を報告することになる）。**入手経路**: `CM-7` / `GPUCOMP-9` で f32 → 表示バイト列の変換が評価ワーカーへ移り UI 側に f32 が無くなったので、**読み取りが有効な間だけ**リニアフレームの CPU コピーを `DisplayFrame` に添えて運ぶ（ホストの `Arc<AtomicBool>` を `DisplayTransform` が毎フレーム読む。チャンネルと同じ形）。**ポインタ移動では再評価も読み戻しも起きない** — UI は手元の `FrameBuffer` を添字するだけで、オーバーレイは評価ターゲットを 1 つも宣言しない。on/off の切り替え時だけ出力段フレームキャッシュを捨てて `InvalidationHint::None` で 1 回要求する（キャッシュ済みの完成フレームは f32 を持たないため）。**対価**: 有効な間は `GPUCOMP-9` が外した 1 画素 16 バイトの読み戻しが戻る。**無効な間は 1 バイトも増えない**のが採用理由で、部分読み戻し（`read_texture` の region 版）はポインタ移動ごとの GPU 同期か UI スレッドでの GPU 待ちになるので採らなかった。コンプ座標 → 評価バッファ添字の換算は両解像度から行う（評価バッファはコンプより小さいことがある）。コンプ枠の外とキャンバス領域の外では何も出さない。状態はセッション内だけで `.ravprj` にも `ui_state.json` にも載らない。ズームルーペは非対象。**実機未確認**（自動テストのみ） |
| 再生とキャッシュの状態表示 | ✅ | 画面右上に 1 行（`·` 区切り）で 「今見えている絵は真実ではない」だけを出す（`INSP-4`、REQ-UI-004）。要素は 3 つ: **ドロップフレーム**（`Transport::dropped_frames`。再生開始でリセットされる累積値なので**再生中だけ**。停止したら消える — 終わった区間のカウントはもう確かめられない）、**プレビュー解像度**（**実効係数が選択係数より低いときだけ** = `VRES-4` の適応降格中だけ。選択係数はツールバーが常時出しており、既定が 1/2 なので「コンプより粗い」で出すと警告が居座って隣のドロップ数まで読まれなくなる。降格は編集とスクラブでも起きるので停止中も出す。`1/4` 選択時は適応が無効なので出ない。判定は**係数どうしの比較**で、画素数の対からは復元しない — `apply` は `div_ceil` なので奇数幅で戻せず、そもそも選択係数は画素数に入っていない）、**キャッシュ済みの割合**（`CACHE-6` のキャッシュ帯 ÷ コンプ長。**再生中だけ**。停止中は Timeline の帯がフレーム単位で同じことを言うので二重表示にしない。帯が空＝評価前は割合を出さず他の要素だけ出る）。3 つとも無ければラベルを 1 枚も置かない。**書き手は 1 経路**（`PlaybackController::publish_position` が `panels::PlaybackStatus` Global を値が変わったときだけ書く）で、**観測者は置かない** — 再生中はプレイヘッドの再描画が毎フレーム走るので、購読すると `HIGH-21` と同型の二重再描画になる。要素の判断と 1 行の組み立ては `ravel_ui::panels::viewer::playback_status_text`（純粋関数）。**実機未確認**（自動テストのみ） |
...
| 画像インスタンスの描画 | ✅ | `geometry.from_image` で包んだ FrameBuffer を `scatter.*` / `geometry.repeat` などで並べた結果が Viewer に出る。CPU 参照経路と GPU 経路の両方が描き、`source_index` でコピーごとに別の画像を選べる。**GPU 経路は GPU 常駐フレームを読み戻さない**（CPU 参照経路だけがソースごとに 1 回読み戻す）。コピーを拡大するとボケるのは仕様（`done/image-instancing-plan.md` 決定 1 / 5）|
...
| GPU テクスチャ共有（ゼロコピー） | ⚠️ | **macOS / Linux / Windows で実装済み**（`ZC-2`〜`ZC-5`、`ZC-7`、`ZC-8`）。macOS は Metal、Linux / Windows は wgpu の出力テクスチャを surface としてそのまま描き、完了通知の後にプールへ返す。Windows は Ravel が GPUI の wgpu/DX12 renderer feature を有効化する。Linux / Windows の実機確認は未了。共有できない場合や context を取得できない場合は評価ワーカーで 1 回読み戻し → `RenderImage`（BGRA u8）の従来経路へフォールバックする。**デバイス喪失は別扱い**: surface 描画は採用したデバイスとレンダラの現デバイスを毎回照合して止まり、ユーザーへはセッションで 1 度だけ通知する（`HIGH-33`）。**Linux / FreeBSD / Windows は復帰する**（`GPULOSS-3`）: 採用元の window のレンダラを 1 秒間隔で polling し、`gpu_device_lost()` が立っている間は zero-copy を切って再描画を促し（GPUI の recovery は Linux / FreeBSD では platform draw の中、Windows では device-lost メッセージの中で走り、後者も復旧後の提示に次の draw を要する。どちらも idle な window は促さないと前に進まない）、レンダラが別デバイスで戻ってきたらそれを `interop::context_from_wgpu` で採り直して `AdoptedHostDevice` を差し替え、`ProjectState::restart_eval_on_gpu`（`GPULOSS-2`）で評価パイプラインを作り直し、交換が始まった場合だけ zero-copy を戻す。**実機未確認**: 実際のデバイス喪失を起こして確認したわけではなく、証明しているのは観測から分岐までの判断（`host_recovery_step` の表）と capability の順序で、cfg の中身は CI のコンパイルしか通っていない（実機確認は `GPULOSS-5`）。なお通知文は喪失の**由来**で分かれる: 採用デバイス（Linux / FreeBSD / Windows）なら「引き継ぐので再起動は不要」、自前デバイス（macOS）なら「このセッションでは復帰しないので再起動を」。**macOS は安全側で確定済み**（`GPULOSS-4`）: Ravel 自前 device の loss callback が立った時点で zero-copy を**セッション中は永久に**無効化し、評価はそのまま続けて CPU フレームを出し続ける。絵は出続けるが**これは復旧ではない** — device は死んだままで、worker も作り直さない（macOS には代わりの device を取る経路が無いので、作り直せば同じ死んだ device に新しい worker を建てることになる）。**GPUI 自身の Metal renderer の喪失を「問う口」は無い**: `gpu_device_lost()` / `gpu_context_full()` は fork の `PlatformWindow` に cfg(linux / freebsd / windows) でしか無く、`gpui_macos` は実装していない（`None` が返るのではなく呼べない）。ただし**別 device への差し替えは毎 paint の照合が捕まえる** — `with_surface_texture` が renderer の現在の device ハンドルを毎回受け取るので、identity が変われば描かずに capability が落ちて CPU 経路へ移る。**残る穴は「identity が変わらないまま device が失われた」場合だけ**で、そのとき macOS は問う手段を持たない（不正なサンプリングは照合が弾くので起きない）。fork に native loss status / epoch の口を足す可否は `MED-APP-40` として別に開けてある |
| ツールバー（選択/ペン等） | ✅ | 選択 / ペン / 矩形 / 楕円 / 多角形 / 星形 / ハンド / ズーム（`ToolState` Global、REQ-UI-011、`tool-system-plan.md`）。多角形 / 星形は押下点を中心にドラッグ距離を外半径として引く（`TOOLX-4`） |
| Hand / Zoom ツール | ✅ | Hand は左ドラッグでパン。**一時ハンド（`H` ホールド）と常時ハンド（ツールバー選択）は同じ経路**で、中ボタンドラッグ（どのツール中でも有効）と同じ 1 つの `PanDrag` を使う。Zoom はクリックで拡大 / `Alt`+クリックで縮小 / ドラッグで矩形ズーム。倍率はスクロールズームと同じ指数の段（1 クリック = ホイール 10 ノッチ相当 ≈ 1.49 倍）で、別の倍率体系を作っていない。アンカーもスクロールズームと同じ「カーソル下のコンプ画素が動かない」規約。矩形ズームは短い側の比で合わせるのでコンプの縦横比は変わらず、矩形の中心がパネル中心へ来る。両軸とも 8px 未満のドラッグはクリック扱い。ドラッグ中は矩形を 1px の枠で描き、`Escape` で取り消す。**Hand / Zoom 中は選択・シェイプ描画・ペン・オーバーレイハンドル・ガイドのどれも始まらない**（左ボタンの分岐は `left_mouse_down` の 1 か所）。`TOOLX-1`、`MED-APP-15` |
| 矩形選択 | ✅ | **Select ツールで何も無い場所からドラッグ**すると矩形選択（`TOOLX-2`）。対象はクリック選択と同じ規約で、**ネットワークが開いていればそのノード、開いていなければアクティブコンプのレイヤー**。判定は箱と**評価済み bbox の交差**（内包ではない。境界は閉じているので接触でも掴む）。**Shift 開始で加算**（押下時点の選択をキャプチャして和を publish するので、Node Editor の `LOW-APP-03` のように既存選択を捨てない）。距離 0 のドラッグはクリック扱いで、押下が publish した内容（空白クリックなら解除）が残る。枠はオーバーレイ機構（`BoxSelectOverlay`）が描く。**未選択の候補の bbox は誰も評価していない**ので、ドラッグ中だけ候補の評価ターゲットを宣言し（開いているネットワークの全 geometry ノード、無ければ全レイヤーの分）、確定はリリース時にその時点の結果で計算する — **結果が届くまでの数フレームは箱の中身が空になりうる**。宣言はジェスチャー中に変わらないのでポインタ移動では評価を要求し直さない（要求は押下の 1 回だけ）。ヒットのある場所からのドラッグは既存の移動ジェスチャーのまま |
| ヒット対象のフォールバック | ✅ | **掴めるものが無かったクリック**（矩形選択が始まった押下を距離 0 で離したとき）が、コンプの**他レイヤーの shape ノード**を上から順に探す（`TOOLX-3`、REQ-UI-011 の v1.5）。見つかればそのノードを選択し、**レイヤー選択もそのレイヤーへ移す**（Outliner のノード行と同じ規約）。アクティブレイヤーでヒットがあれば矩形選択自体が始まらないので、常に全レイヤーを見るコストは払わない。押下時に開いていたネットワークのレイヤーは飛ばす（押下が既に調べている）。探すのは**合成に入るレイヤーだけ**で、muted と（solo があるときの）非 solo は候補にならない（判定はコンポジタの前段と同じ `Composition::composites`）。各レイヤーは自分の殻変換を通して判定するので**変換済みレイヤーも掴める** — これで「2 枚以上選択中に変換済みレイヤーの bbox 内を押すと全部解除される」（`TOOLX-2` の申告した副作用）が解消した。掴んだ後の編集は従来どおり恒等な殻に限る（REQ-UI-011 の v2）。候補の bbox は矩形選択と同じ宣言で測るので、**結果が届く前に離すと空振りする**（既知の制約） |
| パスの点の挿入 / 削除 | ✅ | **ペンツールのときだけ**、選択中の `shape.custom_path` に対してセグメント上のクリックで点を挿入、既存点のクリックでその点を削除する（`TOOLX-3`）。挿入は **de Casteljau の分割**なので曲線の形が変わらない（両隣の向き合う接線を分割後の値に書き換える）。削除は両隣の接線を触らない。点が 2 個のパスは削除を拒否し、そのクリックは消費する（その場に新しいパスは生えない）。掴む半径はパスハンドルと同じ 8px で、優先順位は**接線ハンドル > 削除 > 挿入**（接線の印はアンカーから描かれるので、腕が 8px より短い点では接線が勝つ — 曲げようとした押下が削除になるのを防ぐ。腕を持たない点は接線ハンドルが無いので削除できる）。**クリック 1 回 = 1 undo**（プレビューが無いので押下で即コミット）。選択ツールの挙動（ハンドルのドラッグ）は変えない |
| 接線の対称連動と `Alt` 分離 | ✅ | 滑らかな点の接線をドラッグすると**反対側が鏡像で追従**し、**`Alt` ドラッグは片方だけ**動かす（`TOOLX-3`）。**滑らかかどうかはフラグではなく接線の値で判定する**（`in_tan ≈ -out_tan`。永続化フォーマットは変えない）ので、`Alt` で対称性を壊した点は以後修飾キー無しでも分離済みとして振る舞う。連動させるのは差分なので、わずかに非対称な点が黙って揃えられることはなく、押下点へ戻したドラッグは元の値に戻る。許容差は接線長に対する相対値（1e-4） — f32 のコンプ座標では 10^4 規模で 1 ulp が 10^-3 になるため |
| 選択 bbox とハンドル | ⚠️ | ノード選択（`CanvasSelection`）は評価済みジオメトリから bbox を描く。8 個の印は飾りのままで、ノードのサイズを書く操作は `ParamRole` のマニピュレータが担う。**レイヤー選択が 2 枚以上のときはレイヤー単位 bbox**（そのネットワークの shape ノードの bounds の和 → シェル変換、ハンドル無し。shape ノードを持たないレイヤーは出さない）。ジオメトリを置かないレイヤー（メディア / エフェクトのみ）は bbox 自体が出ない。**測るのは `ravel_core::geometry::ops::drawn_bounds` 1 つ**で、点の位置だけでなくインスタンスが押す source（画像の矩形、グリフの輪郭）と `stroke_width` 属性由来の線の張り出しも入るので、`geometry.from_image` のレイヤーも `text.to_path` を通す前の Text も掴める。**`rasterize` ノードパラメータの基底線幅は入らない**（`LOW-APP-33` の残余）|
| レイヤー殻のマニピュレータ | ✅ | **Select ツールで、ジオメトリを持つレイヤーを 1 枚だけ選んでいるとき**、`ShellManipulator` が殻 bbox に自前の 8 個のスケールハンドル（角は 2 軸、辺の中点は 1 軸）、角の周りに回転リング（描画・当たり判定とも 18px 固定）、中央の移動グリップ、アンカーマーカーを出す。ノード選択 bbox の 8 個の印は飾りで、位置は一致するが別の印である。描画 / ナビゲーションツール中はオーバーレイごと引っ込むので、矩形・楕円・多角形・星形・ペン・ハンド・ズームの押下を奪わない。**Shift はスケールだけ縦横比を固定し、Alt はアンカー基準**（回転の角度スナップと移動の軸拘束は未実装）。アンカーのドラッグは `anchor_point` と `position` を同時に書くので見た目が動かない。キーフレーム付きチャネルは平坦化せず、そのレイヤーのローカルフレームにキーを打ち、親を持つレイヤーは親チェーンの行列込みで表示・編集する。**1 ドラッグ = 1 undo、Esc で revert**。ドラッグ中にレイヤー選択が変わったら押下時のスナップショットへ戻して打ち切る。**読み取りは 2 つに割れている**: 印の位置に「名前 + 現在値（絶対値）」、キャンバス左上に固定の HUD が「`Δ` + 押下からの差分」（スケールは掛かった倍率、回転は掃いた角度、位置とアンカーは符号付きの移動量）。ホバー中は前者だけ、ドラッグ中は両方で、ドラッグ中は掴んでいるハンドルが語る（ホバーは引っ込む）。ホバーの当たり判定はカーソルを決めるものと同じ結果なので、ラベルとカーソルは同じハンドルを指す。ジオメトリを置かないレイヤー（メディア / エフェクトのみ）は bbox とマニピュレータが出ない |
...
| ノードパラメータのマニピュレータ | ✅ | `ParamRole` は `Position` / `Size` の 2 種だけ。選択ノードの評価済みジオメトリにハンドルを出し、アニメーション付きでも現在のローカルフレームへ書く。接続で駆動されるパラメータにはハンドルを出さない。ホバー中とドラッグ中はその印の位置に**パラメータ名（Properties の行と同じ解決）とパラメータ自身の値**を出し（`Size` は描かれる点ではなくオフセットの値）、ドラッグ中は左上に `Δ` と押下からの移動量も出す。押下時の値は index ではなく**キーで突き合わせる**（ハンドル id が指す宣言順はジェスチャ中は動かない前提 — `drag` が置いている前提と同じ — だが、押下時の文書が同じ順で宣言している必要は無い）。`Direction` / `Angle` は未導入 |
| ジオメトリのアトリビュートオーバーレイ | ✅ | 評価済みジオメトリの bbox / points / instances / paths と属性の矢印・インデックス・グループを表示。矢印は実際の `Vec2` / `Vec3` / `Vec4` 属性から作り、picker で選ぶ。ラベルは最大 64 件、矢印の長さ上限はコンプ座標の短辺 15% なので、殻スケールが大きいレイヤーでは画面上で伸びる |
...
| ポインタフィードバック | ✅ | 描画、選択本体、パスアンカー / 接線、ペン閉路位置、**殻のスケール（4 方向の `Resize*`）/ 回転 / アンカー**、定規の帯とガイド線（`ResizeLeftRight` / `ResizeUpDown`）を区別。**Hand は `OpenHand`、パン中は `ClosedHand`、Zoom は `Crosshair`**（`TOOLX-1` でジェスチャーが付いたので割り当てた）。パン / 移動 / 描画 / 殻のドラッグ中もカーソルを維持。動作を持たないノード bbox のハンドルには割り当てない。GPUI-CE に回転カーソルが無いため `DragLink` で代用 |
...
| UI 状態の保存 | ✅ | `ui_state.json`（アクティブコンプ、Timeline の BPM グリッド、コンポジションごとのループ範囲、Properties で畳んだパラメータグループ、ノードエディタのパラメータ値表示、ビューアのプレビュー解像度係数）。任意エントリで、欠落時はそれぞれ `root_comp` フォールバックと `BpmGrid` の既定、ループ範囲なし、**グループは全展開、パラメータ値は描く**、プレビュー解像度は 1/2。既定のままの BPM グリッドとループ範囲ゼロ件、畳んだグループゼロ件、パラメータ値を描く既定、既定の係数はエントリ自体を書かない。プレビュー解像度だけは読めない値でも他のフィールドを失わない（列挙型を足すときの前例、`VRES-3`）。畳みは `(type_key, group)` で覚えるのでノードを選び直しても畳み直さなくてよい。読み込み時に無いコンプの範囲は捨て、Duration の外は引き戻す。既存 v3 アーカイブと互換（format_version 据え置き、REQ-UI-013） |
...
| Document 全体の保存 | ✅ | manifest.json + document/main.ron（Composition・レイヤー・ネットワーク（subnet 入れ子含む）・キーフレーム・予約フィールド・media_assets、決定的 RON。メディアは相対 / 変数パスで記録、公開パラメータ宣言 `exposed_parameters` を含む、format v13）+ settings.toml。保存時に前リビジョンを `.bak` 化。v4 以前のファイルはロード時にベクタパラメータを畳み、v5 以前はカーブパラメータを変換する。v6 以前は宣言ゼロとして読む。v7 以前は作者が指定した色をリニアへ読み替える。v12 以前は `layer.ref` の `layer` を Int から String（参照先 LayerId の十進表記）へ移す。v10 は `ParameterValue::IntChannel`（アニメーション可能な整数）、v11 は `ParameterValue::StringSteps`（アニメーション可能な文字列）、v12 は `Node::param_groups`（In ノードのカスタムパラメータの表示グループ）が入った版で、どれも変換は無く、旧ファイルはそのまま読める。宣言の追加・改名・並べ替え・削除は Properties の公開パラメータセクションから行える（EXPO-5） |
...
`crates/ravel-app/src/splash.rs`（ウィンドウとステージ）、
`crates/ravel-app/src/main.rs` の `bootstrap`（段の実行）、
`assets/splash/splash@2x.png`（意匠）

メインウィンドウが出るまでのディスク読み込みを覆い、いまどの段を走っているかを
1 行で出す。**意匠はアプリが描かない** — ロゴロックアップ・タグライン・
著作権表記は 1680×1092 の PNG（論理 840×546 の 2 倍）に焼き込み済みで、
アプリが載せるのはバージョン行と進捗行の 2 行だけ。テーマに依存しない
固定の白地なので、テーマ読み込み前に開ける。

| 項目 | 状態 | 備考 |
|------|------|------|
| スプラッシュウィンドウ | ✅ | 840×546 固定、タイトルバー無し（`titlebar: None`）、中央配置、リサイズ・最小化・移動不可。プラットフォームが窓を拒んでも起動は続く（`splash::open` が `None` を返すだけ） |
| 進捗行 | ✅ | 段は `splash::StartupStage`（テーマ → 設定 → キーバインド → レイアウト）。`ALL` が `bootstrap` のループそのもので、ラベルは `[splash]` ロケール節 |
| バージョン行 | ✅ | `splash::version_line`（`CARGO_PKG_VERSION` から `v0.1.0`）。焼き込みの著作権表記のすぐ上 |
| 段ごとのフレーム | ✅ | 各段の前に `splash::stage_break`（バックグラウンドタイマ）でメインスレッドを返す。挟まないと全段が 1 フレームに収まり進捗行は一度も描かれない。最低表示時間は 110ms |
| メインへの受け渡し | ✅ | `splash::hand_off_to_main` が「メインを開く → スプラッシュを閉じる」の順を 1 箇所で持つ。`QuitMode::LastWindowClosed` なので逆順は起動即終了になる。`open_main` が失敗した場合もスプラッシュを閉じてから `cx.quit()` |
| 非 macOS | 🔲 | **実機未検証**。`titlebar: None` の意味はプラットフォームで違う（macOS では traffic light とリサイズマスクも落ちる）。Linux の `window_decorations` は既定のまま |
| 進捗の粒度 | 🔲 | 段は 4 つで、段の中の進み具合は出さない。レイアウト復元が長いプロジェクトでも「ワークスペースを復元中…」のまま |
