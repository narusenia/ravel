...
| キャッシュ帯 | ✅ | ルーラー下端に 3px の緑の帯で、出力段フレームキャッシュが持つフレームを描く（`CACHE-6`）。範囲は `cached_ranges(comp, precision, resolution)` そのままで、今表示している解像度のエントリだけ。再生・スクラブで伸び、編集で消える。サブフレーム位置は帯にしない。ディスク層（青）は層が未実装。更新は範囲が変わったときだけグローバルへ書き、パネルは購読せず再描画のついでに読む（`HIGH-21` を作らない） |
