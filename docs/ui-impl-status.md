...
| ノードラベル・説明のロケール化 | ✅ | ラベル / 説明 / パラメータ説明は `node.<type_key>.*` ロケールキーから解決（組み込み 40 型すべて en / ja あり）。キー欠落は `type_key` フォールバック、ユーザーリネームは常に優先。Node Editor / Outliner / 追加メニュー / パレットは同一の解決経路（`ravel-app::node_locale`）。Properties の Node Info は `ravel-ui` が発行したキーを `read_only_value` が翻訳する経路で、登録済み型でキーが欠落した場合に限り生キーが出る（走査テストで到達不能） |
