...
| --- | --- | --- | --- |
| critical | 0 | 4 | — （全件解決） |
| high | 7 | 18 | [high/](high/) — 1件1ファイル |
| medium | 34 | 22 | [medium/](medium/) — 領域別5ファイル |
...
14. **[HIGH-25](high/HIGH-25-compositing-in-display-referred-space.md)**
    色空間変換のコードがリポジトリに 1 行も無く、sRGB エンコード済みの値が
    そのまま合成に入っている。取り込み・Viewer・書き出しの 3 経路が同じ
    素朴変換（`clamp(0,1)*255`）で揃っているので**自己整合していて、
    単純な通しでは誤りが見えない**。壊れているのは合成の数学で、
