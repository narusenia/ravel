...
> **解決済み**: `UIX-7`。Vector 行を `String` / `Enum` と同じ無条件の縦積みに
> した（ラベル行の下に成分行）。成分セルは `flex_1` +
> `ravel_widgets::scrub_input::MIN_WIDTH`（48px）の下限を持ち、成分行は
> `flex_wrap` するので、幅が足りなければ折り返して**値は切らず**ラベルだけが
> 省略される（不変条件 5 の「ラベルは省略、値は省略しない」）。しきい値は
> 導入していない。
> `a_narrow_panel_keeps_every_vector_component_inside_it`（160px / 120px で
> 全成分の左右端とセル幅を `debug_bounds` で採寸）が固定する。
>
> **起票時の前提との差分**:
> - 個票は「いずれの案もパネル幅のしきい値が要る」と書いているが、**要らなかった**。
>   `String` / `Enum` 分岐は既にしきい値なしの縦積みで、同じ形をそのまま使えた
> - 「4成分（`Channel4` 由来）」は**今日は到達不能**。`Channel4` は
>   `PropertyField::Color` になるため（`MED-APP-19`）、`PropertyField::Vector` は
>   `Channel2` / `Channel3` 由来の 2〜3 成分しか存在しない。
>   テストは実在する最大の 3 成分で採寸している
> - はみ出しの主因は `min_w(px(56.0))` だけではない。`ScrubInput` 自身が
>   `min_w(48px)` を持つので、セル側の下限を 0 にしても値が潰れるだけで解決しない
>   （実測: 160px パネルで 37.5px のセル）。48px を下限にして折り返す形にした
